...
class paypalr_googlepay extends base
{
    ...
    protected function captureOrAuthorizePayment(string $payment_source): array
    {
        ...
        // Google Pay always uses capture (final sale)
        $response = $this->ppr->captureOrder($paypal_order_id);

        if ($response === false) {
            $this->log->write('Google Pay: capture failed. ' . Logger::logJSON($this->ppr->getErrorInfo()));
            unset($_SESSION['PayPalRestful']['Order'], $_SESSION['payment']);
    ...
    public function after_process()
    {
        ...
        $message =
            ...
            sprintf(MODULE_PAYMENT_PAYPALR_TRANSACTION_TYPE, $payment_info['payment_type'] ?? 'google_pay') . "\n" .
            $timestamp .
        ...
        $payment_type = $this->orderInfo['payment_info']['payment_type'] ?? 'google_pay';
    ...
    {
        return false; // Google Pay doesn't support separate auth
    }
