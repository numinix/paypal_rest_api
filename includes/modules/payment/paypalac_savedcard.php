<?php
/**
 * paypalac_savedcard.php payment module class for handling Saved Cards via PayPal Advanced Checkout.
 *
...
LanguageCompatibility::load('paypalac_savedcard');
 ...
 */
class paypalac_savedcard extends base
{
    protected function getModuleStatusSetting(): string
    {
        return defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_STATUS') ? MODULE_PAYMENT_PAYPALAC_SAVEDCARD_STATUS : 'False';
    }

    protected function getModuleSortOrder(): ?int
    {
        return defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_SORT_ORDER') ? (int)MODULE_PAYMENT_PAYPALAC_SAVEDCARD_SORT_ORDER : null;
    }

    protected function getModuleZoneSetting(): int
    {
        return defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_ZONE') ? (int)MODULE_PAYMENT_PAYPALAC_SAVEDCARD_ZONE : 0;
    }
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            $this->title = MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_TITLE_ADMIN ?? 'Saved Card';
        } else {
            $this->title = (MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_TITLE_ADMIN ?? 'Saved Cards via PayPal Advanced Checkout') . (($curl_installed === true) ? '' : $this->alertMsg(MODULE_PAYMENT_PAYPALAC_ERROR_NO_CURL ?? 'cURL not installed'));
            $this->description = sprintf(MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_DESCRIPTION ?? 'Saved Cards via PayPal Advanced Checkout (v%s)', self::CURRENT_VERSION);
            ...
            $installed_version = defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION') ? MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION : '0.0.0';
            if ($installed_version !== '0.0.0' && version_compare($installed_version, self::CURRENT_VERSION, '<')) {
                $this->description .= sprintf(
                    ...
                    zen_href_link('paypalac_upgrade.php', 'module=paypalac_savedcard&action=upgrade', 'SSL')
                );
            }
        }
    ...
    protected function tableCheckup()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION') && MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION === $current_version) {
            return;
        }

        // Check for version-specific configuration updates
        if (defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION')) {
            switch (true) {
                // Add future version-specific upgrades here
                // case version_compare(MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION, '1.3.5', '<'):
        ...
        $db->Execute(
            "UPDATE " . TABLE_CONFIGURATION . "
                SET configuration_value = '$current_version',
                    last_modified = now()
              WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION'
              LIMIT 1"
        );
    }
    ...
    protected function getPayPalAdvancedCheckoutApi(): ?PayPalAdvancedCheckoutApi
    {
        ...
        try {
            ...
            $this->log->write('Saved Card: Error creating PayPalAdvancedCheckoutApi: ' . $e->getMessage());
    ...
    public function selection()
    {
        ...
        $selectedVaultId = $_POST['paypalac_savedcard_vault_id'] ?? ($_SESSION['PayPalAdvancedCheckout']['saved_card'] ?? '');
        ...
        $selectLabel = defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_SELECT_LABEL') 
            ? MODULE_PAYMENT_PAYPALAC_SAVEDCARD_SELECT_LABEL 
            : 'Select Card:';
        ...
        $moduleTitle = defined('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_TITLE_SHORT') 
            ? MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_TITLE_SHORT 
            : 'Pay with Saved Card';
        ...
        $savedCardCss = '<link rel="stylesheet" href="' . DIR_WS_MODULES . 'payment/paypal/PayPalAdvancedCheckout/paypalac.css' . '">';
        $checkoutScript = $savedCardCss . '<script defer src="' . DIR_WS_MODULES . 'payment/paypal/PayPalAdvancedCheckout/jquery.paypalac.checkout.js"></script>';
    ...
    {
        return $card['brand'] ?: ($card['card_type'] ?: (MODULE_PAYMENT_PAYPALAC_SAVEDCARD_UNKNOWN_CARD ?? 'Card'));
    }
    ...
    protected function buildCardTitle(array $card): string
    {
        ...
        $cardTitle = sprintf(
            MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_TITLE ?? '%s ending in %s',
        ...
        if (!empty($card['expiry'])) {
            $cardTitle .= sprintf(
                MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_EXPIRY ?? ' (Exp: %s)',
    ...
    protected function getCardBrandImage(string $brand): string
    {
        ...
        if (isset($brandMap[$brandLower])) {
            $imagePath = DIR_WS_MODULES . 'payment/paypal/PayPalAdvancedCheckout/images/' . $brandMap[$brandLower];
    ...
    public function pre_confirmation_check()
    {
        ...
        if (empty($vaultId)) {
            $this->setMessageAndRedirect(
                MODULE_PAYMENT_PAYPALAC_TEXT_SAVED_CARD_NOT_FOUND ?? 'No saved card selected. Please select a card or enter a new one.',
                FILENAME_CHECKOUT_PAYMENT
            );
        }

        $_SESSION['PayPalAdvancedCheckout']['saved_card'] = $vaultId;
        ...
        if ($paypal_order_created === false) {
            ...
            $this->setMessageAndRedirect(
                sprintf(MODULE_PAYMENT_PAYPALAC_TEXT_CREATE_ORDER_ISSUE, MODULE_PAYMENT_PAYPALAC_SAVEDCARD_TEXT_TITLE_ADMIN ?? 'Saved Card', $error_code),
                FILENAME_CHECKOUT_PAYMENT
            );
        }
    }
    ...
    protected function validateSavedCard(string $vaultId): bool
    {
        ...
        foreach ($vaultCards as $card) {
            if ($card['vault_id'] === $vaultId) {
                ...
                $this->ccInfo = [
                    'type' => $card['card_type'] ?? MODULE_PAYMENT_PAYPALAC_SAVEDCARD_UNKNOWN_CARD ?? 'Card',
    ...
    public function process_button()
    {
        ...
        $vaultId = $_SESSION['PayPalAdvancedCheckout']['saved_card'] ?? '';
        return zen_draw_hidden_field('paypalac_savedcard_vault_id', $vaultId);
    }
    ...
    public function process_button_ajax()
    {
        return [
            'ccFields' => [
                'paypalac_savedcard_vault_id' => 'paypalac_savedcard_vault_id',
            ],
        ];
    }
    ...
    public function before_process()
    {
        ...
        if (in_array($payment['status'], ['DECLINED', 'DENIED', 'FAILED'], true)) {
            $this->log->write("==> paypalac_savedcard::before_process: Payment {$payment['status']}; redirecting to checkout.");
            unset($_SESSION['PayPalAdvancedCheckout']['Order'], $_SESSION['payment']);
    ...
    public function check(): bool
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_STATUS'");
    ...
    public function install()
    {
        ...
        $db->Execute(
            "INSERT INTO " . TABLE_CONFIGURATION . "
             ...
             VALUES
                ('Module Version', 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION', '$current_version', 'Currently-installed module version.', 6, 0, 'zen_cfg_read_only(', NULL, now()),
                ('Enable PayPal Saved Cards?', 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_STATUS', 'False', 'Do you want to enable PayPal Saved Card payments? Each saved card will appear as a separate payment option.<br><br><b>Note:</b> This module requires that PayPal Vault is enabled in the main PayPal module and that the Credit Card module is also installed.', 6, 0, 'zen_cfg_select_option([''True'', ''False'', ''Retired''], ', NULL, now()),
                ('Sort order of display.', 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first. Each saved card will be displayed incrementally from this value.', 6, 0, NULL, NULL, now()),
                ('Payment Zone', 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', 6, 0, 'zen_cfg_pull_down_zone_classes(', 'zen_get_zone_class_title', now())"
        );

        // -----
        // Migrate configuration values from the old paypalr_savedcard module if it was previously installed.
        ...
        $old_config = $db->Execute(
            "SELECT configuration_key, configuration_value
              ...
              WHERE configuration_key LIKE 'MODULE\_PAYMENT\_PAYPALR\_SAVEDCARD\_%'"
        );
        while (!$old_config->EOF) {
            $old_key = $old_config->fields['configuration_key'];
            $new_key = str_replace('MODULE_PAYMENT_PAYPALR_SAVEDCARD_', 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_', $old_key);
        ...
        define('MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION', '0.0.0');
        $this->tableCheckup();
    }
    ...
    {
        return [
            'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_VERSION',
            'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_STATUS',
            'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_SORT_ORDER',
            'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_ZONE',
        ];
    }
    ...
    {
        global $db;
        $db->Execute("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key LIKE 'MODULE_PAYMENT_PAYPALAC_SAVEDCARD_%'");
    }
