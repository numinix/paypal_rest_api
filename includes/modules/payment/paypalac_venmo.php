...
class paypalac_venmo extends base
{
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            $this->title = MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE ?? 'PayPal Venmo';
        } else {
            $this->title = (MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE_ADMIN ?? 'PayPal Venmo') . (($curl_installed === true) ? '' : $this->alertMsg(MODULE_PAYMENT_PAYPALAC_ERROR_NO_CURL ?? 'cURL not installed'));
            $this->description = sprintf(MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_DESCRIPTION ?? 'Venmo via PayPal Advanced Checkout (v%s)', self::CURRENT_VERSION);
