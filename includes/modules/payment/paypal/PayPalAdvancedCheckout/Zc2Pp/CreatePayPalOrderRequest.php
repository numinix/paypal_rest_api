 ...
 * for the PayPalAdvancedCheckout (paypalac) Payment Module
 *
...
use PayPalAdvancedCheckout\Zc2Pp\Name;
...
class CreatePayPalOrderRequest extends ErrorInfo
{
    ...
    //
    // Note: The $order_info and $ot_diffs arrays are created by the payment-module's auto.paypaladvcheckout.php observer.
    //
    public function __construct(string $ppac_type, \order $order, array $cc_info, array $order_info, array $ot_diffs)
    {
        ...
        if ($ppac_type === 'card') {
            ...
            $appleWalletPayload = $_SESSION['PayPalAdvancedCheckout']['WalletPayload']['apple_pay'] ?? null;
