<?php
/**
 * PayPalAdvancedCheckoutApi.php communications class for the PayPal Advanced Checkout payment module
 *
 ...
 */
class PayPalAdvancedCheckoutApi extends ErrorInfo
{
    // -----
    ...
    public function __construct(string $endpoint_type, string $client_id = '', string $client_secret = '')
    {
        ...
        if ($client_id === '' || $client_secret === '') {
            $this->log->write(
                sprintf(
                    "PayPalAdvancedCheckoutApi::__construct: REST credentials for the %s environment are not fully configured.",
    ...
    // ---------------------------------------------------------------------
    // PayPal Billing-Subscription cycle sale refunds.
    //
    // Sales generated by a PayPal-managed subscription (BILLING.SUBSCRIPTION.*)
    // are NOT v2 "captures" - PayPal exposes them as v1 "sales" with their
    // own refund endpoint. Calling v2/payments/captures/{sale_id}/refund
    // returns RESOURCE_NOT_FOUND because the id doesn't exist in the v2
    // checkout-capture namespace. We therefore add a parallel pair of helpers
    // that target /v1/payments/sale/{sale_id}/refund and normalize the v1
    // response shape so the rest of the admin refund flow (which expects v2
    // shape) keeps working unchanged. See DoRefund.php for the dispatch.
    // ---------------------------------------------------------------------
    public function refundSaleFull(string $paypal_sale_id, string $invoice_id, string $payer_note)
    {
        return $this->refundSale($paypal_sale_id, $invoice_id, $payer_note);
    }
    public function refundSalePartial(string $paypal_sale_id, string $currency_code, string $value, string $invoice_id, string $payer_note)
    {
        return $this->refundSale($paypal_sale_id, $invoice_id, $payer_note, ['currency' => $currency_code, 'total' => $value]);
    }
    protected function refundSale(string $paypal_sale_id, string $invoice_id, string $payer_note, array $amount = [])
    {
        $this->log->write("==> Start refundSale($paypal_sale_id, $invoice_id, $payer_note, ...)\n" . Logger::logJSON($amount), true);
        $parameters = [];
        // PayPal v1 sale refund field-length limits diverge from the v2 capture
        // refund:
        //   - description    : max 127 chars (human-readable note, what we want
        //                       for the operator's free-form payer note)
        //   - reason         : max 30 chars  (intended for short reason codes;
        //                       PayPal returns VALIDATION_ERROR if exceeded)
        //   - invoice_number : max 127 chars
        // The admin UI's "Note to payer" text comes from the same field used by
        // v2 (note_to_payer / max 255). Long notes blow up "reason"; route them
        // to "description" instead, and only use "reason" for a short label.
        if ($invoice_id !== '') {
            $parameters['invoice_number'] = substr($invoice_id, 0, 127);
        }
        if ($payer_note !== '') {
            $parameters['description'] = mb_substr($payer_note, 0, 127);
        }
        // Use a fixed, short reason code so we always stay within the 30-char
        // PayPal limit; the operator's actual note is in `description`.
        $parameters['reason'] = 'Refund issued by admin';
        if (!empty($amount)) {
            $parameters['amount'] = $amount;
        }
        $response = $this->curlPost("v1/payments/sale/$paypal_sale_id/refund", $parameters);
        $this->log->write("==> End refundSale", true);
        if (!is_array($response)) {
            return $response;
        }
        // Normalize v1 -> v2 shape so DoRefund.php and addDbTransaction handlers
        // (which read $response['id'], $response['status'], $response['amount']['*'],
        // $response['create_time'], $response['update_time'], and $response['links'])
        // work without branching. Specifically GetPayPalOrderTransactions::
        // addDbTransaction reaches into $response['links'] to derive the parent
        // (rel='up') transaction id; if the key is missing it fatals with a
        // TypeError, which surfaces as a 500 on the orders refund page even
        // though the refund itself succeeded at PayPal.
        $state = strtoupper((string)($response['state'] ?? ''));
        // V1 refund returns 'sale' link but not 'up'; synthesize an 'up' link
        // pointing at the sale we just refunded so addDbTransaction can recover
        // the parent txn id automatically.
        $normalizedLinks = [];
        $saleLinkHref = '';
        foreach (($response['links'] ?? []) as $link) {
            ...
            if ($rel === 'sale' && $saleLinkHref === '') {
                $saleLinkHref = (string)($link['href'] ?? '');
            }
            $normalizedLinks[] = $link;
        }
        // Fallback: build the sale link from the id we POSTed against, since
        // PayPal sometimes omits the sale link in sandbox.
        if ($saleLinkHref === '' && $paypal_sale_id !== '') {
            $saleLinkHref = 'https://api.paypal.com/v1/payments/sale/' . $paypal_sale_id;
        }
        if ($saleLinkHref !== '') {
            $normalizedLinks[] = [
                'rel' => 'up',
        ...
        $normalized = [
            ...
            'status' => $state === 'COMPLETED' ? 'COMPLETED' : ($state === 'PENDING' ? 'PENDING' : $state),
            'amount' => [
                'value' => (string)($response['amount']['total'] ?? $response['total_refunded_amount']['value'] ?? '0.00'),
                'currency_code' => (string)($response['amount']['currency'] ?? $response['amount']['currency_code'] ?? ''),
            ],
            'create_time' => (string)($response['create_time'] ?? gmdate('Y-m-d\TH:i:s\Z')),
            'update_time' => (string)($response['update_time'] ?? $response['create_time'] ?? gmdate('Y-m-d\TH:i:s\Z')),
            'links' => $normalizedLinks,
            'final_capture' => false,
            // Preserve original payload for any downstream code that wants it.
            '_v1_original' => $response,
        ];
        return $normalized;
    }

    /**
     * Retrieve the status of a v1 sale (used as a parallel to getCaptureStatus
     * for billing-subscription cycle refunds). Returns v2-shaped data so the
     * admin transactions list can update the parent transaction status using
     * the same code path as v2 captures.
     */
    public function getSaleStatus(string $paypal_sale_id)
    {
        $this->log->write("==> Start getSaleStatus ($paypal_sale_id)", true);
        $response = $this->curlGet("v1/payments/sale/$paypal_sale_id");
        $this->log->write("==> End getSaleStatus", true);
        if (!is_array($response)) {
            return $response;
        }
        $state = strtoupper((string)($response['state'] ?? ''));
        return [
            'id' => (string)($response['id'] ?? ''),
            'status' => $state === 'COMPLETED' ? 'COMPLETED' : ($state === 'PARTIALLY_REFUNDED' ? 'PARTIALLY_REFUNDED' : ($state === 'REFUNDED' ? 'REFUNDED' : $state)),
            'amount' => [
                'value' => (string)($response['amount']['total'] ?? '0.00'),
                'currency_code' => (string)($response['amount']['currency'] ?? ''),
            ],
            'update_time' => (string)($response['update_time'] ?? ''),
            'create_time' => (string)($response['create_time'] ?? ''),
            '_v1_original' => $response,
        ];
    }
