(function () {
    ...
    function setPaylaterPayload(payload) {
        var payloadField = document.getElementById('paypalr-paylater-payload');
        if (payloadField) {
            try {
        ...
        var statusField = document.getElementById('paypalr-paylater-status');
        if (statusField) {
    ...
    function selectPaylaterRadio() {
        var moduleRadio = document.getElementById('pmt-paypalr_paylater');
     ...
     * This hides the parent element (e.g., paypalr_paylater-custom-control-container)
     * so the user doesn't see an unavailable payment option.
     */
    function hidePaymentMethodContainer() {
        var container = document.getElementById('paypalr-paylater-button');
        if (!container) {
            return;
        }
        ...
        var parentContainer = container.closest('[id*="paypalr_paylater"][id*="container"]') 
            || container.closest('.moduleRow')
            || container.closest('[class*="paypalr_paylater"]');
        ...
        while (parent && depth < maxDepth) {
            ...
            if (parentId.indexOf('paypalr_paylater') !== -1 || 
                parentClass.indexOf('paypalr_paylater') !== -1 ||
    ...
    function rerenderPaylaterButton() {
        if (typeof window.paypalrPaylaterRender === 'function') {
            window.paypalrPaylaterRender();
        }

        if (typeof document !== 'undefined' && typeof document.dispatchEvent === 'function') {
            document.dispatchEvent(new CustomEvent('paypalr:paylater:rerender'));
        }
    }
    ...
    function renderPaylaterButton() {
        ...
        fetchWalletConfig().then(function (config) {
            ...
            return loadPayPalSdk(config).then(function (paypal) {
                ...
                var buttonInstance = paypal.Buttons({
                    ...
                    onApprove: function (data) {
                        ...
                        return cachePaylaterPayload(payload).finally(function () {
                            ...
                            document.dispatchEvent(new CustomEvent('paypalr:paylater:payload', { detail: payload }));
                        });
                    },
                    onCancel: function (data) {
                        console.warn('Pay Later cancelled', data);
                        setPaylaterPayload({});
                        document.dispatchEvent(new CustomEvent('paypalr:paylater:payload', { detail: {} }));
                    },
                ...
                return buttonInstance.render('#paypalr-paylater-button');
            });
    ...
    window.paypalrPaylaterSetPayload = setPaylaterPayload;
    window.paypalrPaylaterSelectRadio = selectPaylaterRadio;

    document.addEventListener('paypalr:paylater:payload', function (event) {
    ...
    if (container) {
        ...
        if (container.innerHTML.trim() === '') {
            container.innerHTML = '<span class="paypalr-paylater-placeholder">' + (typeof paypalrPaylaterText !== 'undefined' ? paypalrPaylaterText : 'Pay Later') + '</span>';
            normalizeWalletButton(container.firstElementChild);
        }
    }

    if (typeof window !== 'undefined') {
        window.paypalrPaylaterRender = renderPaylaterButton;
    }
