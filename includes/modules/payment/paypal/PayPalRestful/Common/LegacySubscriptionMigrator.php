...
class LegacySubscriptionMigrator
{
    ...
    private static function upsertSubscription(array $data): void
    {
        ...
        if ((int)$record['orders_products_id'] === 0) {
            unset($record['orders_products_id']);
        }

        $existing = null;

        if ((int)$data['legacy_subscription_id'] > 0) {
            $existing = $db->Execute(
        ...
        if (($existing === null || $existing->EOF) && $data['profile_id'] !== '') {
            $existing = $db->Execute(
                "SELECT paypal_subscription_id FROM " . TABLE_PAYPAL_SUBSCRIPTIONS
        ...
        if (($existing === null || $existing->EOF) && (int)$data['orders_products_id'] > 0) {
            $existing = $db->Execute(
                'SELECT paypal_subscription_id FROM ' . TABLE_PAYPAL_SUBSCRIPTIONS
