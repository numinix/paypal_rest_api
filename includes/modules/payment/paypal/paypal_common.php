...
class PayPalCommon {
    ...
    public function processWalletConfirmation($walletType, $payloadFieldName, $errorMessages) {
        ...
        // -----------------------------------------------------------------
        // Apple Pay special case:
        //
        // PayPal is currently returning INTERNAL_SERVICE_ERROR (500) when
        // /confirm-payment-source is used with apple_pay. To avoid that,
        // we create a fresh order that *already* includes the Apple Pay
        // token as its payment_source and skip confirmPaymentSource.
        //
        // Flow for apple_pay now:
        //   1. JS collects token & contacts and posts payload.
        //   2. normalizeWalletPayload() JSON-encodes the token and stores
        //      it in $_SESSION['PayPalRestful']['WalletPayload']['apple_pay'].
        //   3. We clear any previous wallet order (from fetchWalletOrder).
        //   4. We create a NEW order; CreatePayPalOrderRequest sees the
        //      token in the session and injects:
        //          payment_source.apple_pay.token
        //   5. captureWalletPayment() later calls captureOrder() and the
        //      payment succeeds without using confirmPaymentSource.
        // -----------------------------------------------------------------
        if ($walletType === 'apple_pay') {
            // Throw away any earlier wallet order that may have been
            // created by the JS fetchWalletOrder() call.
            ...
            // IMPORTANT: Skip confirmPaymentSource for apple_pay.
            return;
        }

        // -----------------------------------------------------------------
        // Non-Apple Pay wallets (Google Pay, Venmo, etc):
        // existing createOrder + confirmPaymentSource flow.
        // -----------------------------------------------------------------
        $paypal_order_created = $this->paymentModule->createPayPalOrder($walletType);
        if ($paypal_order_created === false) {
            $error_info = $this->paymentModule->ppr->getErrorInfo();
