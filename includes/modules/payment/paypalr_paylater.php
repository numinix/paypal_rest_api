<?php
/**
 * paypalr_paylater.php payment module class for handling Pay Later via PayPal Advanced Checkout.
 *
...
LanguageCompatibility::load('paypalr_paylater');

/**
 * The PayPal Pay Later payment module using PayPal's REST APIs (v2)
 */
class paypalr_paylater extends base
{
    protected function getModuleStatusSetting(): string
    {
        return defined('MODULE_PAYMENT_PAYPALR_PAYLATER_STATUS') ? MODULE_PAYMENT_PAYPALR_PAYLATER_STATUS : 'False';
    }

    protected function getModuleSortOrder(): ?int
    {
        return defined('MODULE_PAYMENT_PAYPALR_PAYLATER_SORT_ORDER') ? (int)MODULE_PAYMENT_PAYPALR_PAYLATER_SORT_ORDER : null;
    }

    protected function getModuleZoneSetting(): int
    {
        return defined('MODULE_PAYMENT_PAYPALR_PAYLATER_ZONE') ? (int)MODULE_PAYMENT_PAYPALR_PAYLATER_ZONE : 0;
    }
    ...
    public function __construct()
    {
        ...
        $this->code = 'paypalr_paylater';

        $curl_installed = (function_exists('curl_init'));

        if (IS_ADMIN_FLAG === false) {
            $this->title = MODULE_PAYMENT_PAYPALR_PAYLATER_TEXT_TITLE ?? 'PayPal Pay Later';
        } else {
            $this->title = (MODULE_PAYMENT_PAYPALR_PAYLATER_TEXT_TITLE_ADMIN ?? 'PayPal Pay Later') . (($curl_installed === true) ? '' : $this->alertMsg(MODULE_PAYMENT_PAYPALR_ERROR_NO_CURL ?? 'cURL not installed'));
            $this->description = sprintf(MODULE_PAYMENT_PAYPALR_PAYLATER_TEXT_DESCRIPTION ?? 'Pay Later via PayPal Advanced Checkout (v%s)', self::CURRENT_VERSION);
            
            // Add upgrade button if current version is less than latest version
            $installed_version = defined('MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION') ? MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION : '0.0.0';
            if ($installed_version !== '0.0.0' && version_compare($installed_version, self::CURRENT_VERSION, '<')) {
                $this->description .= sprintf(
                    ...
                    zen_href_link('paypalr_upgrade.php', 'module=paypalr_paylater&action=upgrade', 'SSL')
                );
            }
        }
        ...
        // is to be performed. Pay Later is a wallet payment (ppr_type !== 'card').
        //
        ...
        if (!defined('MODULE_PAYMENT_PAYPALR_VERSION')) {
            $this->enabled = false;
            if (IS_ADMIN_FLAG === true) {
                $this->title .= $this->alertMsg(MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_PAYPAL_REQUIRED ?? 'Main PayPal module required');
            }
            return;
        }
    }
    ...
    protected function tableCheckup()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION') && MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION === $current_version) {
            return;
        }

        // -----
        // Check if the messaging configuration needs to be migrated from the old paypalr module.
        // This happens when the paylater module is newly installed but the messaging setting
        // already exists from the main paypalr module.
        //
        ...
        $db->Execute(
            "UPDATE " . TABLE_CONFIGURATION . "
                SET configuration_value = '$current_version',
                    last_modified = now()
              WHERE configuration_key = 'MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION'
              LIMIT 1"
        );
    }
    ...
    protected function getPayPalRestfulApi(): ?PayPalRestfulApi
    {
        ...
        try {
            ...
            $this->log->write('Pay Later: Error creating PayPalRestfulApi: ' . $e->getMessage());
    ...
    public function selection(): array
    {
        ...
        $buttonContainer = '<div id="paypalr-paylater-button" class="paypalr-paylater-button"></div>';
        $hiddenFields =
            zen_draw_hidden_field('ppr_type', 'paylater') .
            zen_draw_hidden_field('paypalr_paylater_payload', '', 'id="paypalr-paylater-payload"') .
            zen_draw_hidden_field('paypalr_paylater_status', '', 'id="paypalr-paylater-status"');

        $script = $this->getWalletAssets('jquery.paypalr.paylater.js');

        return [
            'id' => $this->code,
            'module' => MODULE_PAYMENT_PAYPALR_PAYLATER_TEXT_SELECTION ?? 'Pay Later',
            'fields' => [
                [
    ...
    public function ajaxGetWalletConfig(): array
    {
        ...
        if ($client_id === '') {
            ...
            return ['success' => false, 'message' => MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_INITIALIZE ?? 'Unable to start Pay Later. Please try again.'];
        }
    ...
    public function ajaxCreateWalletOrder(): array
    {
        ...
        if ($response['success'] === false && empty($response['message'])) {
            $response['message'] = MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_INITIALIZE ?? 'Unable to start Pay Later. Please try again.';
        }
    ...
    public function pre_confirmation_check()
    {
        $this->paypalCommon->processWalletConfirmation(
            'paylater',
            'paypalr_paylater_payload',
            [
                'title' => MODULE_PAYMENT_PAYPALR_PAYLATER_TEXT_TITLE ?? 'Pay Later',
                'payload_missing' => MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_PAYLOAD_MISSING ?? 'Payload missing',
                'payload_invalid' => MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_PAYLOAD_INVALID ?? 'Invalid payload',
                'confirm_failed' => MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_CONFIRM_FAILED ?? 'Confirmation failed',
                'payer_action' => MODULE_PAYMENT_PAYPALR_PAYLATER_ERROR_PAYER_ACTION ?? 'Payer action required',
            ]
        );
    }
    ...
    public function before_process()
    {
        ...
        // This is especially important when using Pay Later from cart/product page where
     ...
     * This ensures the order has valid customer data when using Pay Later
    ...
    protected function populateOrderCustomerInfo($order, array $paypalResponse)
    {
        ...
        $payment_source = $paypalResponse['payment_source']['paylater'] ?? [];
        ...
        if ($email_address !== '') {
            ...
            $this->log->write("Pay Later: Updated order customer email to: " . $email_address);
        }
        ...
        if ($first_name !== '' || $last_name !== '') {
            ...
            $this->log->write("Pay Later: Updated order customer name to: " . $first_name . " " . $last_name);
        }
    ...
    public function check(): bool
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_PAYPALR_PAYLATER_STATUS'");
    ...
    public function install()
    {
        ...
        $db->Execute(
            "INSERT INTO " . TABLE_CONFIGURATION . "
                (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
             VALUES
                ('Module Version', 'MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION', '$current_version', 'Currently-installed module version.', 6, 0, 'zen_cfg_read_only(', NULL, now()),
                ('Enable PayPal Pay Later?', 'MODULE_PAYMENT_PAYPALR_PAYLATER_STATUS', 'False', 'Do you want to enable PayPal Pay Later payments? Pay Later allows customers to pay in installments (Pay in 4) or with monthly financing. Available in USD, GBP, EUR, and AUD only.', 6, 0, 'zen_cfg_select_option([''True'', ''False'', ''Retired''], ', NULL, now()),
                ('PayLater Messaging', 'MODULE_PAYMENT_PAYPALR_PAYLATER_MESSAGING', 'Checkout, Shopping Cart, Product Pages', 'On which pages should PayPal PayLater messaging be displayed? (It will automatically not be displayed in regions where it is not available. Only available in USD, GBP, EUR, AUD.) When enabled, it will show the lower installment-based pricing for the presented product or cart amount. This may accelerate buying decisions.<br><b>Default: All</b>', 6, 0, 'zen_cfg_select_multioption([''Checkout'', ''Shopping Cart'', ''Product Pages'', ''Product Listings and Search Results''], ', NULL, now()),
                ('Sort order of display.', 'MODULE_PAYMENT_PAYPALR_PAYLATER_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', 6, 0, NULL, NULL, now()),
                ('Payment Zone', 'MODULE_PAYMENT_PAYPALR_PAYLATER_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', 6, 0, 'zen_cfg_pull_down_zone_classes(', 'zen_get_zone_class_title', now())" 
        );
        
        // Define the module's current version so that the tableCheckup method will apply all changes
        define('MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION', '0.0.0');
        $this->tableCheckup();
    }
    ...
    {
        return [
            'MODULE_PAYMENT_PAYPALR_PAYLATER_VERSION',
            'MODULE_PAYMENT_PAYPALR_PAYLATER_STATUS',
            'MODULE_PAYMENT_PAYPALR_PAYLATER_MESSAGING',
            'MODULE_PAYMENT_PAYPALR_PAYLATER_SORT_ORDER',
            'MODULE_PAYMENT_PAYPALR_PAYLATER_ZONE',
        ];
    }
