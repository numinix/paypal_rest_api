...
class paypalr_venmo extends base
{
    ...
    protected function captureOrAuthorizePayment(string $payment_source): array
    {
        ...
        // Venmo always uses capture (final sale)
        $response = $this->ppr->captureOrder($paypal_order_id);

        if ($response === false) {
            $this->log->write('Venmo: capture failed. ' . Logger::logJSON($this->ppr->getErrorInfo()));
            unset($_SESSION['PayPalRestful']['Order'], $_SESSION['payment']);
    ...
    public function after_process()
    {
        ...
        $message =
            ...
            sprintf(MODULE_PAYMENT_PAYPALR_TRANSACTION_TYPE, $payment_info['payment_type'] ?? 'venmo') . "\n" .
            $timestamp .
        ...
        $payment_type = $this->orderInfo['payment_info']['payment_type'] ?? 'venmo';
