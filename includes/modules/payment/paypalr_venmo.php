...
LanguageCompatibility::load('paypalr_venmo');
...
class paypalr_venmo extends base
{
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            ...
            $installed_version = defined('MODULE_PAYMENT_PAYPALR_VENMO_VERSION') ? MODULE_PAYMENT_PAYPALR_VENMO_VERSION : '0.0.0';
        ...
        // is to be performed. Venmo is a wallet payment (ppr_type !== 'card').
        //
    ...
    protected function tableCheckup()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALR_VENMO_VERSION') && MODULE_PAYMENT_PAYPALR_VENMO_VERSION === $current_version) {
            return;
        }
        ...
        if (defined('MODULE_PAYMENT_PAYPALR_VENMO_VERSION')) {
            switch (true) {
                    ...
                    if (!defined('MODULE_PAYMENT_PAYPALR_VENMO_ACCOUNT_ID') && $account_id_setting->EOF) {
                        $db->Execute(
                            "INSERT INTO " . TABLE_CONFIGURATION . "
                             ...
                             VALUES
                                ('Venmo Business Profile ID', 'MODULE_PAYMENT_PAYPALR_VENMO_ACCOUNT_ID', '', 'Enter the Venmo business profile or account ID tied to your PayPal Advanced Checkout credentials. This value is required for Venmo transactions.', 6, 0, NULL, NULL, now())"
                        );
                    }
                    // Fall through to update the stored version number

                case version_compare(MODULE_PAYMENT_PAYPALR_VENMO_VERSION, '1.3.5', '<'):
                    $db->Execute(
                ...
                case version_compare(MODULE_PAYMENT_PAYPALR_VENMO_VERSION, '1.3.6', '<'):
                    $db->Execute(
                        "UPDATE " . TABLE_CONFIGURATION . "
        ...
        $db->Execute(
            "UPDATE " . TABLE_CONFIGURATION . "
                SET configuration_value = '$current_version',
                    last_modified = now()
              WHERE configuration_key = 'MODULE_PAYMENT_PAYPALR_VENMO_VERSION'
              LIMIT 1"
        );
    }
    ...
    public function selection(): array
    {
        ...
        $script = $this->getWalletAssets('jquery.paypalr.venmo.js');
    ...
    public function ajaxCreateWalletOrder(): array
    {
        ...
        if ($response['success'] === false && empty($response['message'])) {
            $response['message'] = MODULE_PAYMENT_PAYPALR_VENMO_ERROR_INITIALIZE ?? 'Unable to start Venmo. Please try again.';
        }
    ...
    public function before_process()
    {
        ...
        // This is especially important when using Venmo from cart/product page where
     ...
     * This ensures the order has valid customer data when using Venmo
    ...
    protected function populateOrderCustomerInfo($order, array $paypalResponse)
    {
        ...
        $payment_source = $paypalResponse['payment_source']['venmo'] ?? [];
        ...
        if ($email_address !== '') {
            ...
            $this->log->write("Venmo: Updated order customer email to: " . $email_address);
        }
        ...
        if ($first_name !== '' || $last_name !== '') {
            ...
            $this->log->write("Venmo: Updated order customer name to: " . $first_name . " " . $last_name);
        }
    ...
    public function install()
    {
        ...
        $db->Execute(
            "INSERT INTO " . TABLE_CONFIGURATION . "
                (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
             VALUES
                ('Module Version', 'MODULE_PAYMENT_PAYPALR_VENMO_VERSION', '$current_version', 'Currently-installed module version.', 6, 0, 'zen_cfg_read_only(', NULL, now()),
                ...
                ('Venmo Business Profile ID', 'MODULE_PAYMENT_PAYPALR_VENMO_ACCOUNT_ID', '', 'Optional Venmo business profile ID reserved for future Venmo buttons; not required for PayPal-managed Venmo checkout.', 6, 0, NULL, NULL, now()),
        ...
        define('MODULE_PAYMENT_PAYPALR_VENMO_VERSION', '0.0.0');
        $this->tableCheckup();
    }
    ...
    {
        return [
            'MODULE_PAYMENT_PAYPALR_VENMO_VERSION',
