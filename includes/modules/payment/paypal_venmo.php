<?php
/**
 * paypal_venmo.php payment module class for handling Venmo via PayPal Advanced Checkout.
 *
...
class paypal_venmo extends paypalr
{
    /**
    ...
    public function selection(): array
    {
        ...
        $buttonContainer = '<div id="paypal-venmo-button" class="paypal-venmo-button"></div>';
        $hiddenFields =
            zen_draw_hidden_field('ppr_type', 'venmo') .
            zen_draw_hidden_field('paypal_venmo_payload', '', 'id="paypal-venmo-payload"') .
            zen_draw_hidden_field('paypal_venmo_status', '', 'id="paypal-venmo-status"');

        $script = '<script>' . file_get_contents(DIR_WS_MODULES . 'payment/paypal/PayPalRestful/jquery.paypal.venmo.js') . '</script>';
    ...
    public function pre_confirmation_check()
    {
        ...
        $payloadRaw = $_POST['paypal_venmo_payload'] ?? '';
        if (trim($payloadRaw) === '') {
