...
LanguageCompatibility::load('paypalr_applepay');

/**
 * The PayPal Apple Pay payment module using PayPal's REST APIs (v2)
 */
class paypalr_applepay extends base
{
    protected function getModuleStatusSetting(): string
    {
        return defined('MODULE_PAYMENT_PAYPALR_APPLEPAY_STATUS') ? MODULE_PAYMENT_PAYPALR_APPLEPAY_STATUS : 'False';
    }

    protected function getModuleSortOrder(): ?int
    {
        return defined('MODULE_PAYMENT_PAYPALR_APPLEPAY_SORT_ORDER') ? (int)MODULE_PAYMENT_PAYPALR_APPLEPAY_SORT_ORDER : null;
    }
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            $this->title = MODULE_PAYMENT_PAYPALR_APPLEPAY_TEXT_TITLE ?? 'PayPal Apple Pay';
        } else {
            $this->title = (MODULE_PAYMENT_PAYPALR_APPLEPAY_TEXT_TITLE_ADMIN ?? 'PayPal Apple Pay') . (($curl_installed === true) ? '' : $this->alertMsg(MODULE_PAYMENT_PAYPALR_ERROR_NO_CURL ?? 'cURL not installed'));
            $this->description = sprintf(MODULE_PAYMENT_PAYPALR_APPLEPAY_TEXT_DESCRIPTION ?? 'Apple Pay via PayPal Advanced Checkout (v%s)', self::CURRENT_VERSION);
            ...
            if ($installed_version !== '0.0.0' && version_compare($installed_version, self::CURRENT_VERSION, '<')) {
                $this->description .= sprintf(
                    ...
                    zen_href_link('paypalr_upgrade.php', 'module=paypalr_applepay&action=upgrade', 'SSL')
                );
            }
        }
        ...
        // is to be performed. Apple Pay is a wallet payment (ppr_type !== 'card').
        //
        ...
        if (!defined('MODULE_PAYMENT_PAYPALR_VERSION')) {
            $this->enabled = false;
            if (IS_ADMIN_FLAG === true) {
                $this->title .= $this->alertMsg(MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_PAYPAL_REQUIRED ?? 'Main PayPal module required');
            }
            return;
        }
    }
    ...
    protected function tableCheckup()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALR_APPLEPAY_VERSION') && MODULE_PAYMENT_PAYPALR_APPLEPAY_VERSION === $current_version) {
            return;
        }
        ...
        $db->Execute(
            "UPDATE " . TABLE_CONFIGURATION . "
                SET configuration_value = '$current_version',
                    last_modified = now()
              WHERE configuration_key = 'MODULE_PAYMENT_PAYPALR_APPLEPAY_VERSION'
              LIMIT 1"
        );
    }
    ...
    public function selection(): array
    {
        ...
        $script = $this->getWalletAssets('jquery.paypalr.applepay.js');

        return [
            'id' => $this->code,
            'module' => MODULE_PAYMENT_PAYPALR_APPLEPAY_TEXT_SELECTION ?? 'Apple Pay',
            'fields' => [
                [
    ...
    public function ajaxGetWalletConfig(): array
    {
        ...
        if ($client_id === '') {
            ...
            return ['success' => false, 'message' => MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_INITIALIZE ?? 'Unable to start Apple Pay. Please try again.'];
        }
    ...
    public function ajaxCreateWalletOrder(): array
    {
        ...
        if ($response['success'] === false && empty($response['message'])) {
            $response['message'] = MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_INITIALIZE ?? 'Unable to start Apple Pay. Please try again.';
        }
    ...
    public function pre_confirmation_check()
    {
        $this->paypalCommon->processWalletConfirmation(
            ...
            [
                'title' => MODULE_PAYMENT_PAYPALR_APPLEPAY_TEXT_TITLE ?? 'Apple Pay',
                'payload_missing' => MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_PAYLOAD_MISSING ?? 'Payload missing',
                'payload_invalid' => MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_PAYLOAD_INVALID ?? 'Invalid payload',
                'confirm_failed' => MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_CONFIRM_FAILED ?? 'Confirmation failed',
                'payer_action' => MODULE_PAYMENT_PAYPALR_APPLEPAY_ERROR_PAYER_ACTION ?? 'Payer action required',
            ]
        );
    }
    ...
    public function before_process()
    {
        ...
        // This is especially important when using Apple Pay from cart/product page where
     ...
     * This ensures the order has valid customer data when using Apple Pay
    ...
    protected function populateOrderCustomerInfo($order, array $paypalResponse)
    {
        ...
        $payment_source = $paypalResponse['payment_source']['apple_pay'] ?? [];
        ...
        if ($email_address !== '') {
            ...
            $this->log->write("Apple Pay: Updated order customer email to: " . $email_address);
        }
        ...
        if ($first_name !== '' || $last_name !== '') {
            ...
            $this->log->write("Apple Pay: Updated order customer name to: " . $first_name . " " . $last_name);
        }
    ...
    protected function recordPayPalOrderDetails(int $orders_id): void
    {
        ...
        // Implementation similar to paypalr but adapted for Apple Pay
        if ($orders_id <= 0) {
            return;
        }
    ...
    public function check(): bool
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_PAYPALR_APPLEPAY_STATUS'");
    ...
    public function install()
    {
        ...
        $db->Execute(
            "INSERT INTO " . TABLE_CONFIGURATION . "
                (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
             VALUES
                ('Module Version', 'MODULE_PAYMENT_PAYPALR_APPLEPAY_VERSION', '$current_version', 'Currently-installed module version.', 6, 0, 'zen_cfg_read_only(', NULL, now()),
        ...
        define('MODULE_PAYMENT_PAYPALR_APPLEPAY_VERSION', '0.0.0');
        $this->tableCheckup();
    }
    ...
    {
        return [
            'MODULE_PAYMENT_PAYPALR_APPLEPAY_VERSION',
    ...
    {
        global $db;
        $db->Execute("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key LIKE 'MODULE_PAYMENT_PAYPALR_APPLEPAY_%'");
    }
