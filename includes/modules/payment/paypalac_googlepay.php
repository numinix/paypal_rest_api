...
class paypalac_googlepay extends base
{
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            $this->title = MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE ?? 'PayPal Google Pay';
        } else {
            $this->title = (MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE_ADMIN ?? 'PayPal Google Pay') . (($curl_installed === true) ? '' : $this->alertMsg(MODULE_PAYMENT_PAYPALAC_ERROR_NO_CURL ?? 'cURL not installed'));
            $this->description = sprintf(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_DESCRIPTION ?? 'Google Pay via PayPal Advanced Checkout (v%s)', self::CURRENT_VERSION);
