 ...
 * Venmo button template for product page
 * 
 ...
 */
    ...
    require_once(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/paypalac_venmo.php');
...
window.paypalacVenmoSessionAppend = window.paypalacVenmoSessionAppend || "<?php echo zen_session_id() ? ('?' . zen_session_name() . '=' . zen_session_id()) : ''; ?>";
...
(function() {
    ...
    // Function to add product to cart before creating order
    function addProductToCart() {
        // Get product ID and quantity
        const productId = '<?php echo (int)$_GET['products_id']; ?>';
        const qtyField = document.querySelector('input[name="cart_quantity"]');
        ...
        if (form) {
            ...
            inputs.forEach(function(input) {
                if (input.name && input.name !== 'cart_quantity') {
                    formData.append(input.name, input.value);
                }
            });
        }
        
        return fetch('ajax/paypalac_wallet.php' + window.paypalacVenmoSessionAppend, {
            method: 'POST',
    ...
    const venmoButtons = paypal.Buttons({
        ...
        createOrder: function() {
            ...
            // First add product to cart, then create order
        ...
        onApprove: function(data) {
            ...
            return fetch('ajax/paypalac_wallet_checkout.php' + window.paypalacVenmoSessionAppend, {
                method: 'POST',
                credentials: 'same-origin',
                headers: { 
            ...
            .then(function(json) {
                ...
                if (json.status === 'success') {
                    // Clear cart
                    fetch('ajax/paypalac_wallet_clear_cart.php' + window.paypalacVenmoSessionAppend, {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ module: 'paypalac_venmo' })
                    }).finally(function() {
