...
class zcObserverPaypaladvcheckout
{
    ...
    protected function isWalletSdkPage(string $currentPage): bool
    {
        $checkoutPages = [
        ...
        if (defined('FILENAME_CHECKOUT_ONE') && FILENAME_CHECKOUT_ONE !== '') {
            $checkoutPages[] = FILENAME_CHECKOUT_ONE;
        }
        if (defined('FILENAME_CHECKOUT_ONE_CONFIRMATION') && FILENAME_CHECKOUT_ONE_CONFIRMATION !== '') {
            $checkoutPages[] = FILENAME_CHECKOUT_ONE_CONFIRMATION;
        }
        if (defined('FILENAME_QUICK_CHECKOUT') && FILENAME_QUICK_CHECKOUT !== '') {
            $checkoutPages[] = FILENAME_QUICK_CHECKOUT;
        }

        $checkoutPages = array_unique(array_merge($checkoutPages, $this->getNuminixOpcPageKeys()));

        return in_array($currentPage, $checkoutPages, true);
    }
    ...
    protected function outputJsSdkHeaderAssets($current_page): void
    {
        ...
        // -----
        // Don't load the SDK (or write SDK-configuration logs) on 404 pages.
        // These requests can be generated by bots/crawlers and do not represent
        ...
        //
        // Keep wallet-related components scoped to checkout-related pages so the SDK
        // doesn't run site-wide when it isn't needed.
        //
        $components = [];
        $js_page_type = $this->getMessagesPageType();

        // Only include Pay Later messaging where the current page is messageable.
        if (!empty($js_page_type) && $js_page_type !== 'None') {
            $components[] = 'messages';
        }

        // Wallet components are only needed during checkout flows.
        if ($this->isWalletSdkPage((string)$current_page)) {
        ...
        $js_fields['components'] = implode(',', $components);
