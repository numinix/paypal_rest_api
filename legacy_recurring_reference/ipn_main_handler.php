...
//      zen_redirect(zen_href_link(FILENAME_TIME_OUT));
    ...
    if ($isDPtransaction && empty($_SESSION['payment'])) {
        // Ensure the expected WPP payment module is available so that order creation can continue.
        $_SESSION['payment'] = 'paypalwpp';
    }

    if ($isDPtransaction && empty($_SESSION['shipping'])) {
        // IPN callbacks might not have a shipping method stored; initialise with a safe default.
        $_SESSION['shipping'] = '';
    }
    ...
    if ($_POST['payment_type'] === 'instant' && $isDPtransaction && ((isset($_POST['auth_status']) && $_POST['auth_status'] === 'Completed') || $_POST['payment_status'] === 'Completed')) {
        ipn_debug_email('IPN NOTICE :: DP/Website Payments Pro notice -- continuing to ensure order creation.');
    }
    ...
    switch ($txn_type) {
        ...
        case ($posted_txn_type === 'paypal_here'):
            ...
            die();
            break;
        case (str_starts_with($posted_txn_type, 'subscr_') && $posted_txn_type !== 'subscr_payment'):
            // Filter out subscription lifecycle notifications we don't yet consume
            ipn_debug_email('IPN NOTICE :: Subscription lifecycle notice - Not currently supported by Zen Cart. IPN handler aborted.');
            die();
            break;
        ...
        case ($posted_txn_type === 'subscr_payment'):
            ipn_debug_email('IPN NOTICE :: Subscription payment received - continuing to create order.');
        ...
        case 'unique':
            ...
            if (!str_contains($txn_type, 'denied') && !str_contains($txn_type, 'failed') && !str_contains($txn_type, 'voided')) {
                ...
                // modified by numinix for plugin licensing
                $zco_notifier->notify(
