...
/**
 * Test that validates the paypalac_creditcard module's process_button_ajax method
...
echo "Testing paypalac_creditcard::process_button_ajax() method...\n\n";
...
$all_passed &= assert_array_has_key('ppac_saved_card', $result['ccFields'], "Has ppac_saved_card field");
$all_passed &= assert_array_has_key('ppac_cc_owner', $result['ccFields'], "Has ppac_cc_owner field");
$all_passed &= assert_array_has_key('ppac_cc_number', $result['ccFields'], "Has ppac_cc_number field");
$all_passed &= assert_array_has_key('ppac_cc_cvv', $result['ccFields'], "Has ppac_cc_cvv field");
$all_passed &= assert_array_has_key('ppac_cc_expires_month', $result['ccFields'], "Has ppac_cc_expires_month field");
$all_passed &= assert_array_has_key('ppac_cc_expires_year', $result['ccFields'], "Has ppac_cc_expires_year field");
$all_passed &= assert_equals('paypalac_cc_owner', $result['ccFields']['ppac_cc_owner'], "Correct mapping for cc_owner");
echo "\n";

echo "Test 2: process_button_ajax includes save card field when checked...\n";
$_POST = ['paypalac_saved_card' => 'new', 'paypalac_cc_save_card' => 'on'];
$module = new paypalac_creditcard();
$result = $module->process_button_ajax();
$all_passed &= assert_array_has_key('ppac_cc_save_card', $result['ccFields'], "Has ppac_cc_save_card field when checkbox is checked");
echo "\n";
...
$module = new paypalac_creditcard();
$result = $module->process_button_ajax();
$all_passed &= assert_array_not_has_key('ppac_cc_save_card', $result['ccFields'], "Does not have ppac_cc_save_card field when checkbox is not checked");
echo "\n";

echo "Test 4: process_button_ajax includes SCA field when present...\n";
$_POST = ['paypalac_saved_card' => 'new', 'paypalac_cc_sca_always' => '1'];
$module = new paypalac_creditcard();
$result = $module->process_button_ajax();
$all_passed &= assert_array_has_key('ppac_cc_sca_always', $result['ccFields'], "Has ppac_cc_sca_always field when present in POST");
echo "\n";

echo "Test 5: process_button_ajax returns minimal fields for saved card...\n";
$_POST = ['paypalac_saved_card' => 'vault-id-123'];
$module = new paypalac_creditcard();
$result = $module->process_button_ajax();
$all_passed &= assert_array_has_key('ppac_saved_card', $result['ccFields'], "Has ppac_saved_card field for saved card");
$all_passed &= assert_array_not_has_key('ppac_cc_owner', $result['ccFields'], "Does not have ppac_cc_owner field for saved card");
$all_passed &= assert_array_not_has_key('ppac_cc_number', $result['ccFields'], "Does not have ppac_cc_number field for saved card");
echo "\n";
...
$module = new paypalac_creditcard();
$result = $module->process_button_ajax();
$all_passed &= assert_array_has_key('ppac_cc_owner', $result['ccFields'], "Defaults to new card fields when no selection");
$all_passed &= assert_array_has_key('ppac_cc_number', $result['ccFields'], "Defaults to new card fields when no selection");
echo "\n";
