 ...
 * use the centralized PayPalAdvancedCheckoutApi class which automatically includes the partner attribution
...
namespace {
    ...
    if (!defined('PayPalAdvancedCheckout\Api\PayPalAdvancedCheckoutApi::PARTNER_ATTRIBUTION_ID')) {
        fwrite(STDERR, "FAILED: PARTNER_ATTRIBUTION_ID constant is not defined\n");
        $failures++;
    } else {
        $partnerId = PayPalAdvancedCheckoutApi::PARTNER_ATTRIBUTION_ID;
    ...
    try {
        $api = new PayPalAdvancedCheckoutApi('sandbox', 'TestClientId', 'TestClientSecret');
    ...
    /**
     * Test 5: Verify other code locations use PayPalAdvancedCheckoutApi
     */
    ...
    foreach ($otherLocations as $locationName => $locationPath) {
        ...
        if (strpos($locationContents, 'new PayPalAdvancedCheckoutApi') === false && strpos($locationContents, 'new \PayPalAdvancedCheckout\Api\PayPalAdvancedCheckoutApi') === false) {
            fwrite(STDERR, "WARNING: $locationName does not instantiate PayPalAdvancedCheckoutApi (may use it indirectly)\n");
        } else {
            echo "  ✓ $locationName uses PayPalAdvancedCheckoutApi (includes partner attribution)\n";
        }
    }
    ...
    if ($failures > 0) {
        ...
        echo "  through centralized code in the PayPalAdvancedCheckoutApi class. The partner\n";
