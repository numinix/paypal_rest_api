...
class ApplePayAuthorizeModeOrderStatusTest
{
    ...
    private function testConstructorLogicMatchesPaypalac(): void
    {
        ...
        if ($applePayHasFinalSale && $applePayHasCardOnly && $applePayHasOrderStatus && $applePayHasPendingStatus &&
            ...
            $this->testResults[] = [
                ...
                'passed' => true,
                'message' => 'Apple Pay constructor uses same transaction mode logic as main paypalac module'
            ];
            ...
            $this->testResults[] = [
                'name' => 'Constructor logic matches paypalac',
                'passed' => false,
                'message' => 'Logic differs between Apple Pay and main paypalac module'
            ];
    ...
    private function testBeforeProcessHandlesCreatedStatus(): void
    {
        ...
        if (preg_match($pattern, $content)) {
            ...
            $oldPattern = '/if\s*\(\s*\$payment_status\s*!==\s*PayPalAdvancedCheckoutApi::STATUS_CAPTURED\s*&&\s*\$payment_status\s*!==\s*PayPalAdvancedCheckoutApi::STATUS_CREATED\s*\)/s';
