...
define('HELPERS_FILE_PATH', DIR_FS_CATALOG . 'numinix.com/includes/modules/pages/paypal_signup/includes/nxp_paypal_helpers.php');
...
{
    // Find the function signature - support both functions with and without return type declarations
    $pattern = '/function\s+' . preg_quote($funcName, '/') . '\s*\([^)]*\)(?:\s*:\s*\w+)?\s*\{/';
...
{
    $passed = true;
    $content = file_get_contents(HELPERS_FILE_PATH);
