...
namespace {
    ...
    if (!isset($purchase_unit['amount']['breakdown']['discount'])) {
        fwrite(STDERR, "Expected discount element for unknown discount module.\n");
        $failures++;
    } elseif ((float)$purchase_unit['amount']['breakdown']['discount']['value'] !== 1.00) {
        fwrite(STDERR, sprintf("Expected discount of 1.00, got %s.\n", $purchase_unit['amount']['breakdown']['discount']['value']));
        $failures++;
    }
    ...
    fwrite(STDOUT, "CreatePayPalOrderRequest generic discount module test passed.\n");
}
