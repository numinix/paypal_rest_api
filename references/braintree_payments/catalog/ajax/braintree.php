...
if (!isset($_SESSION['cart']) || !is_object($_SESSION['cart'])) {
    ...
    log_braintree_message('Session/cart object missing when accessing ajax/braintree.php');
...
if ($zone_count === 1) {
    ...
    $zone_query = $db->Execute("
        SELECT zone_id FROM " . TABLE_ZONES . "
        WHERE zone_country_id = " . (int)$country_id . "
        AND zone_code = '" . zen_db_input($administrativeArea) . "'
        LIMIT 1
    ");
    
    if ($zone_query->RecordCount() > 0) {
        $zone_id = (int)$zone_query->fields['zone_id'];
        log_braintree_message("Matched zone for $administrativeArea: zone_id=$zone_id (exact zone_code match)");
    } else {
        // Try matching against zone_name if zone_code didn't match (e.g., 'Alabama', 'California')
        $zone_name_query = $db->Execute("
            SELECT zone_id FROM " . TABLE_ZONES . "
            WHERE zone_country_id = " . (int)$country_id . "
            AND zone_name = '" . zen_db_input($administrativeArea) . "'
            LIMIT 1
        ");
        
        if ($zone_name_query->RecordCount() > 0) {
            $zone_id = (int)$zone_name_query->fields['zone_id'];
            log_braintree_message("Matched zone for $administrativeArea: zone_id=$zone_id (exact zone_name match)");
        } else {
            log_braintree_message("No exact zone_code or zone_name match for administrativeArea=$administrativeArea, using zone_id=0");
        }
    }
}
