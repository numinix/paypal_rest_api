...
if ($incomingSessionParam !== null && !empty($_GET[$incomingSessionParam])) {
    ...
    if (!isset($_COOKIE['cookie_test'])) {
        $_COOKIE['cookie_test'] = 'braintree_session_bridge';
    }
}
...
if (function_exists('zen_load_language')) {
    ...
    braintree_load_language_file('checkout_process');
...
$braintreeCheckoutErrorResponder = function ($message) use (&$braintreeCheckoutErrorHandled) {
    ...
    braintree_checkout_log($message);
...
log_braintree_message("Payload total from client: $total (currency: $currency)");
...
// Determine if guest flow should be used based on admin configuration
// Wallet modules include Google Pay, Apple Pay, and PayPal
$walletModules = ['braintree_googlepay', 'braintree_applepay', 'braintree_paypal'];
$isWalletModule = in_array($module, $walletModules, true);

// Check admin setting for wallet user account type (default to 'Guest' for backward compatibility)
$walletAccountType = defined('MODULE_PAYMENT_BRAINTREE_WALLET_USER_ACCOUNT_TYPE')
    ? MODULE_PAYMENT_BRAINTREE_WALLET_USER_ACCOUNT_TYPE
    : 'Guest';
$isGuestCheckout = $isWalletModule && $walletAccountType === 'Guest';
...
if (!isset($_SESSION['customer_id'])) {
    if ($customer_query->RecordCount() > 0) {
        ...
        // If the COWOA_account column exists and guest checkout is enabled, set it to 1
        if ($isGuestCheckout) {
            $check_cowoa_account = $db->Execute("SHOW COLUMNS FROM " . TABLE_CUSTOMERS . " LIKE 'COWOA_account'");
            if ($check_cowoa_account->RecordCount() > 0) {
                $db->Execute("UPDATE " . TABLE_CUSTOMERS . " SET COWOA_account = 1 WHERE customers_id = " . (int)$customer_id);
            }
        }
    ...
    if ($isGuestCheckout) {
        // Treat the wallet flow as a guest checkout but still set the core customer id
    ...
    // Track express checkout module if applicable (for analytics)
    if ($isWalletModule) {
...
log_braintree_message("Shipping address lookup: administrativeArea='" . ($shipping_address['administrativeArea'] ?? '') . "', countryCode='" . ($shipping_address['countryCode'] ?? '') . "', resolved zone_id=$shipping_zone_id, zone_name='$shipping_zone_name'");
...
if ($address_query->RecordCount() > 0) {
    ...
    log_braintree_message("Found existing shipping address ID $address_id: existing zone_id=$existing_zone_id, existing state='$existing_state'");
    ...
    if ($existing_zone_id !== $shipping_zone_id) {
        ...
        log_braintree_message("Updating shipping address ID $address_id: changing zone_id from $existing_zone_id to $shipping_zone_id, state to '$entry_state'");
    ...
    log_braintree_message("Creating shipping address: administrativeArea='" . ($shipping_address['administrativeArea'] ?? '') . "', countryCode='" . ($shipping_address['countryCode'] ?? '') . "', zone_id=$shipping_zone_id, zone_name='$shipping_zone_name', entry_state='$entry_state'");
...
log_braintree_message("Billing address lookup: administrativeArea='" . ($billing_address['administrativeArea'] ?? '') . "', countryCode='" . ($billing_address['countryCode'] ?? '') . "', resolved zone_id=$billing_zone_id, zone_name='$billing_zone_name'");
...
if ($billing_address_query->RecordCount() > 0) {
    ...
    log_braintree_message("Found existing billing address ID $billing_address_id: existing zone_id=$existing_billing_zone_id, existing state='$existing_billing_state'");
    ...
    if ($existing_billing_zone_id !== $billing_zone_id) {
        ...
        log_braintree_message("Updating billing address ID $billing_address_id: changing zone_id from $existing_billing_zone_id to $billing_zone_id, state to '$billing_entry_state'");
    ...
    log_braintree_message("Creating billing address: administrativeArea='" . ($billing_address['administrativeArea'] ?? '') . "', countryCode='" . ($billing_address['countryCode'] ?? '') . "', zone_id=$billing_zone_id, zone_name='$billing_zone_name', entry_state='$billing_entry_state'");
...
if (isset($order->delivery['zone_id']) && $order->delivery['zone_id'] > 0) {
    ...
    if ($delivery_zone_name !== '') {
        ...
        log_braintree_message("Fixed delivery state: zone_id=" . $order->delivery['zone_id'] . ", state set to '" . $delivery_zone_name . "'");
    }
}
...
if (isset($order->billing['zone_id']) && $order->billing['zone_id'] > 0) {
    ...
    if ($billing_zone_name !== '') {
        ...
        log_braintree_message("Fixed billing state: zone_id=" . $order->billing['zone_id'] . ", state set to '" . $billing_zone_name . "'");
    }
}
