...
class braintree_api extends base {
                ...
                (function(){
                    ...
                    function loadBraintreeHostedFieldsScripts(callback) {
                        ...
                        const scripts = [
                            ...
                            \"https://js.braintreegateway.com/web/3.133.0/js/three-d-secure.js\",
                            \"https://js.braintreegateway.com/web/3.133.0/js/hosted-fields.js\"
                        ];
                        ...
                        console.log('Credit Card: Browser detection - iOS Chrome:', isIOSChrome);
                            ...
                            return new Promise(function(resolve, reject) {
                                ...
                                if (existing) {
                                    if (existing.dataset.loaded === 'true') {
                                        console.log('Credit Card: Script already loaded', src);
                                        resolve();
                                        return;
                                    }
                                    ...
                                    if (existing.readyState === 'complete' || existing.readyState === 'loaded') {
                                        ...
                                        console.log('Credit Card: Script already loaded (from cache)', src);
                                        resolve();
                                        return;
                                    }
                                    existing.addEventListener('load', function() {
                                        existing.dataset.loaded = 'true';
                                        console.log('Credit Card: Script loaded', src);
                                        resolve();
                                    });
                                ...
                                script.addEventListener('load', function() {
                                    ...
                                    console.log('Credit Card: Loaded script', src);
                                    resolve();
                                });
                        ...
                        // iOS Chrome requires sequential loading to avoid race conditions
                        // All other browsers work fine with parallel loading (faster)
                        let loadPromise;
                        if (isIOSChrome) {
                            console.log('Credit Card: Using sequential loading for iOS Chrome');
                            loadPromise = scripts.reduce(function(promise, src) {
                                return promise.then(function() {
                                    return loadScript(src);
                                });
                            }, Promise.resolve());
                        } else {
                            console.log('Credit Card: Using parallel loading for non-iOS Chrome browsers');
                            loadPromise = Promise.all(scripts.map(function(src) {
                                return loadScript(src);
                            }));
                        }
                        ...
                        loadPromise.then(function() {
                            ...
                            console.error('Credit Card: Failed to load Hosted Fields scripts', error);
                    ...
                    function initializeBraintreeHostedFields() {
                        ...
                        braintree.client.create({authorization: authorizationValue})
                        ...
                        function startBraintreeTokenization() {
                            ...
                            if (err.code === 'HOSTED_FIELDS_FIELDS_INVALID') {
                                ...
                                invalidKeys.forEach(function(fieldKey) {
                                    ...
                                    const container = fieldState && fieldState.container;
    ...
    public function after_process() {
            ...
            if ($currency && strtoupper($currency) !== strtoupper($sessionCurrency)) {
                $sessionRate = $currencies->get_value($sessionCurrency);
                $settleRate = $currencies->get_value($currency);
                
                if ($sessionRate > 0) {
                    // Exchange rate from session currency to settlement currency
                    $exchangeRate = $settleRate / $sessionRate;
                }
            }
