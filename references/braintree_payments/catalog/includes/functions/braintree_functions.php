...
if (!function_exists('braintree_load_language_file')) {
    function braintree_load_language_file($basename)
    {
...
/**
 * Look up zone ID by zone code for a given country.
 * Only performs exact zone_code matches (e.g., 'PA', 'CA').
 * If no exact match is found, returns 0 to let Zen Cart handle the state name as-is.
 * 
 * @param string $zone_code The zone code to look up (e.g., 'PA', 'CA', 'WA')
 * @param string $country_code The country ISO code (e.g., 'US')
 * @param object $db The database connection object
 * @return int The zone ID, or 0 if not found
 */
function braintree_lookup_zone_id($zone_code, $country_code, $db) {
    ...
    if ($zone_count === 1) {
        ...
        $zone_query = $db->Execute("
            SELECT zone_id FROM " . TABLE_ZONES . "
            WHERE zone_country_id = " . $country_id . "
            AND zone_code = '" . zen_db_input($zone_code) . "'
            LIMIT 1
        ");
...
function braintree_normalize_contact_value($value) {
    if (!is_string($value)) {
        return $value;
    }
...
function braintree_sanitize_contact_array(&$contact) {
    if (!is_array($contact)) {
        return;
    }
    ...
    array_walk_recursive($contact, function (&$value) {
        if (is_string($value)) {
            $value = braintree_normalize_contact_value($value);
        }
    });
}
...
function normalize_braintree_contact($contact, $module = '') {
    ...
    braintree_sanitize_contact_array($contact);
...
function braintree_get_common_instance() {
    static $instance = null;
    static $attempted = false;

    if ($instance instanceof BraintreeCommon) {
        return $instance;
    }
    ...
    if (!defined('MODULE_PAYMENT_BRAINTREE_SERVER')
    ...
    $config = [
        ...
        'debug_logging' => defined('MODULE_PAYMENT_BRAINTREE_DEBUGGING')
            && MODULE_PAYMENT_BRAINTREE_DEBUGGING !== 'Alerts Only',
    ];

    try {
        $instance = new BraintreeCommon($config);
    } catch (\Throwable $exception) {
        error_log('Braintree configuration error: ' . $exception->getMessage());
        $instance = null;
    }
...
function braintree_generate_client_token($currency = null) {
    static $cachedTokens = [];
    ...
    $common = braintree_get_common_instance();
    if (!$common) {
        return '';
    }
...
function braintree_get_tokenization_key() {
    $common = braintree_get_common_instance();
