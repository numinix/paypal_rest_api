...
if (!$_SESSION['cart']->count_contents() > 0 || (isset($_SESSION['gv_no']) && $_SESSION['gv_no'] != '')) {
    $my_account = true;
}
...
if (isset($_REQUEST['oprcaction']) && ($_REQUEST['oprcaction'] == 'process')) {
    if (isset($_GET['autologin']) && $_GET['autologin'] == 'true' && OPRC_EASY_SIGNUP_AUTOMATIC_LOGIN == 'true' && isset($_COOKIE['email_address']) && isset($_COOKIE['password'])) {
        $auto_login = true;
    ...
    if (!$auto_login) {
        ...
        if (strlen($email_address) < (int) ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
            $error = true;
            $messageStack->add_session(
                'login',
                defined('OPRC_ENTRY_EMAIL_ADDRESS_ERROR')
                    ? OPRC_ENTRY_EMAIL_ADDRESS_ERROR
                    : (defined('ENTRY_EMAIL_ADDRESS_ERROR') ? ENTRY_EMAIL_ADDRESS_ERROR : 'Please enter a valid email address.')
            );
        } elseif (!zen_validate_email($email_address)) {
            $error = true;
            $messageStack->add_session(
                'login',
                defined('OPRC_ENTRY_EMAIL_ADDRESS_CHECK_ERROR')
                    ? OPRC_ENTRY_EMAIL_ADDRESS_CHECK_ERROR
                    : (defined('ENTRY_EMAIL_ADDRESS_CHECK_ERROR') ? ENTRY_EMAIL_ADDRESS_CHECK_ERROR : 'Please enter a valid email address.')
            );
        }
        ...
        if (strlen($password) < (int) ENTRY_PASSWORD_MIN_LENGTH) {
            ...
            $messageStack->add_session(
                'login',
                defined('OPRC_ENTRY_PASSWORD_ERROR')
                    ? OPRC_ENTRY_PASSWORD_ERROR
                    : (defined('ENTRY_PASSWORD_ERROR') ? ENTRY_PASSWORD_ERROR : 'Please enter a valid password.')
            );
        }
    }
    ...
    if ($error !== true
        && !$auto_login
        && (
            (!isset($_SESSION['securityToken']) || !isset($_POST['securityToken']))
            || ($_SESSION['securityToken'] !== $_POST['securityToken'])
        )
        && (
            PROJECT_VERSION_MAJOR > 1
            || (PROJECT_VERSION_MAJOR == 1 && substr(PROJECT_VERSION_MINOR, 0, 3) >= 3.8)
        )
    ) {
        $error = true;
        ...
        $check_customer_query = "SELECT customers_id, customers_firstname, customers_lastname, customers_password,
                                      ...
                                      customers_authorization, customers_referral, COWOA_account, customers_default_shipping_address_id
        ...
        if (!$check_customer->RecordCount()) {
            ...
            while (!$check_customer->EOF) {
                ...
                if (
                    (!$customer)
                    && (OPRC_MASTER_PASSWORD == 'false'
                        ||  (OPRC_MASTER_PASSWORD == 'true' && (isset($administrator) && !$administrator))
                    )
                ) {
                    if ($check_customer->fields['COWOA_account'] == 0) {
                        $full_account_exists = true;
                    }
            ...
            if ($customer || (isset($administrator) && $administrator && OPRC_MASTER_PASSWORD == 'true')) {
                if (isset($administrator) && $administrator && OPRC_MASTER_PASSWORD == 'true') {
                    $_SESSION['master_password'] = true;
                }
                $ProceedToLogin = true;
                $check_customer_query = "SELECT customers_id, customers_firstname, customers_lastname, customers_password,
                                          customers_email_address, customers_default_address_id,
                                          customers_authorization, customers_referral, customers_default_shipping_address_id
                                   FROM " . TABLE_CUSTOMERS . "
...
if (isset($min_password_failed) && $min_password_failed && isset($_SESSION['customer_id'])) {
    ...
    if ($my_account && isset($_SESSION['customer_id'])) {
        ...
        if (SHOW_SHOPPING_CART_COMBINED > 0 && (isset($zc_check_basket_before) && $zc_check_basket_before > 0)) {
            $zc_check_basket_after = $_SESSION['cart']->count_contents();
            if (
                ($zc_check_basket_before != $zc_check_basket_after)
                && $_SESSION['cart']->count_contents() > 0
                && SHOW_SHOPPING_CART_COMBINED > 0
            ) {
