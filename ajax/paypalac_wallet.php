...
function get_validated_base_currency($currencies, $preferredCurrency = null) {
    ...
    if (isset($currencies->currencies['USD'])) {
        log_paypalac_wallet_message('WARNING: DEFAULT_CURRENCY not available, falling back to USD');
        return 'USD';
    }
    ...
    if (!empty($availableCurrencies)) {
        ...
        log_paypalac_wallet_message('WARNING: Neither DEFAULT_CURRENCY nor USD available, using ' . $fallbackCurrency);
        return $fallbackCurrency;
    }
    
    // This should never happen in a properly configured Zen Cart
    log_paypalac_wallet_message('ERROR: No currencies available in Zen Cart configuration!');
...
if (!isset($_SESSION['cart']) || !is_object($_SESSION['cart'])) {
    ...
    log_paypalac_wallet_message('Session/cart object missing when accessing ajax/braintree.php');
...
log_paypalac_wallet_message('Display currency: ' . $displayCurrency . ', currency value: ' . $displayCurrencyValue);

$module = isset($data['module']) ? $data['module'] : 'braintree_api';
switch ($module) {
    case 'paypalac_googlepay':
        $paymentModuleDebugConstant = 'MODULE_PAYMENT_PAYPALAC_GOOGLE_PAY_DEBUGGING';
        break;
    case 'paypalac_applepay':
        $paymentModuleDebugConstant = 'MODULE_PAYMENT_PAYPALAC_APPLE_PAY_DEBUGGING';
        break;
    case 'paypalac_venmo':
        $paymentModuleDebugConstant = 'MODULE_PAYMENT_PAYPALAC_VENMO_DEBUGGING';
        break;
    default:
        $paymentModuleDebugConstant = 'MODULE_PAYMENT_PAYPALAC_DEBUGGING';
        break;
}
...
if ($zone_count === 1) {
    ...
    log_paypalac_wallet_message("Only one zone found for country $countryCode, using zone_id=$zone_id");
    ...
    if ($zone_query->RecordCount() > 0) {
        ...
        log_paypalac_wallet_message("Matched zone for $administrativeArea: zone_id=$zone_id");
    } else {
        log_paypalac_wallet_message("No matching zone for administrativeArea=$administrativeArea");
    }
}
...
log_paypalac_wallet_message('Session shipping vars: ' . print_r([
...
log_paypalac_wallet_message('Initial order object created in base currency: ' . $baseCurrency . 
...
log_paypalac_wallet_message('Raw shipping quotes returned: ' . print_r($quotes, true));
...
if (!isset($data['selectedShippingOptionId']) || empty($data['selectedShippingOptionId']) || $data['selectedShippingOptionId'] === 'shipping_option_unselected') {
    ...
    log_paypalac_wallet_message('Cheapest shipping method returned: ' . print_r($cheapest, true));
    ...
    if (!empty($cheapest) && !empty($cheapest['id'])) {
        ...
        log_paypalac_wallet_message('Selected cheapest shipping method from cheapest(): ' . $selectedShippingOption);
    }
    
    // If cheapest() didn't work, manually find the cheapest from quotes
    if ($selectedShippingOption === null && !empty($quotes)) {
        log_paypalac_wallet_message('cheapest() did not return valid result, manually finding cheapest from quotes');
        $lowestCost = null;
        $lowestOptionId = null;
        ...
        if ($lowestOptionId !== null) {
            ...
            log_paypalac_wallet_message('Selected cheapest shipping method manually: ' . $selectedShippingOption . ' (cost: ' . $lowestCost . ')');
        }
    }
    
    // Final fallback: use first method if we still don't have one
    if ($selectedShippingOption === null && !empty($quotes)) {
        log_paypalac_wallet_message('No cheapest method found, falling back to first method');
        foreach ($quotes as $quote) {
            if (!empty($quote['error']) || empty($quote['methods'])) continue;
            $firstMethod = $quote['methods'][0];
            $selectedShippingOption = "{$quote['id']}_{$firstMethod['id']}";
            log_paypalac_wallet_message('Selected first available shipping method: ' . $selectedShippingOption);
            break;
        }
    }
    
    if ($selectedShippingOption === null) {
        log_paypalac_wallet_message('No shipping methods available');
    }
} else {
    $selectedShippingOption = $data['selectedShippingOptionId'];
    log_paypalac_wallet_message('Using provided shipping option: ' . $selectedShippingOption);
}
...
foreach ($quotes as $quote) {
    ...
    foreach ($quote['methods'] as $method) {
        ...
        log_paypalac_wallet_message('Shipping option: ' . $optionId . 
            ', base cost: ' . $baseCost . 
            ', tax rate: ' . $taxRate . 
        ...
        if ($optionId === $selectedShippingOption) {
            ...
            log_paypalac_wallet_message('Selected shipping method: ' . $optionId . 
                ', base cost: ' . $baseCost . 
                ', tax rate: ' . $taxRate . 
                ', tax-inclusive base: ' . $taxInclusiveCostBase .
                ', display amount (' . $displayCurrency . '): ' . $shippingDisplayAmount);
...
log_paypalac_wallet_message('Shipping session data for order totals: ' . print_r($_SESSION['shipping'], true));

log_paypalac_wallet_message('Shipping options returned: ' . print_r($shippingOptions, true));
...
log_paypalac_wallet_message('Customer session variables for tax: customer_country_id=' . $country_id . ', customer_zone_id=' . $zone_id);
...
log_paypalac_wallet_message('Order object created in base currency: ' . $baseCurrency . 
...
if (isset($order->products) && is_array($order->products) && function_exists('zen_get_tax_rate')) {
    log_paypalac_wallet_message('Manually calculating product taxes for ' . count($order->products) . ' product(s)');
    ...
    foreach ($order->products as $index => $product) {
        ...
        log_paypalac_wallet_message('  Product ' . ($index + 1) . ' (' . ($product['name'] ?? 'N/A') . '): tax_class_id=' . $taxClassId . ', tax_rate=' . $taxRate . '%');
    }
    
    log_paypalac_wallet_message('Product tax calculation complete');
}

// Debug: Log products in the order (after tax calculation)
if (isset($order->products) && is_array($order->products)) {
    log_paypalac_wallet_message('Order contains ' . count($order->products) . ' product(s):');
    foreach ($order->products as $index => $product) {
        log_paypalac_wallet_message('  Product ' . ($index + 1) . ': ' . print_r([
            'id' => $product['id'] ?? 'N/A',
    ...
    log_paypalac_wallet_message('Order products array is empty or not set');
}

// Debug: Log billing and delivery addresses
log_paypalac_wallet_message('Billing address set: ' . print_r([
...
log_paypalac_wallet_message('Delivery address set: ' . print_r([
    'firstname' => $addressData['firstname'] ?? 'N/A',
    'lastname' => $addressData['lastname'] ?? 'N/A',
    'street_address' => $addressData['street_address'] ?? 'N/A',
    'suburb' => $addressData['suburb'] ?? 'N/A',
...
if (isset($order->products) && is_array($order->products)) {
    log_paypalac_wallet_message('Product taxes before order_totals->process():');
    foreach ($order->products as $index => $product) {
        log_paypalac_wallet_message('  Product ' . ($index + 1) . ': ' . print_r([
            'tax' => $product['tax'] ?? 'N/A',
            'tax_class_id' => $product['tax_class_id'] ?? 'N/A',
...
log_paypalac_wallet_message('Order delivery address before order_totals->process(): ' . print_r([
...
foreach ($order_totals as &$ot) {
    ...
    if ($missingProductTax > 0.0001) {
        ...
        log_paypalac_wallet_message('Product tax missing from ot_tax; adding ' . $missingProductTax . ' (base).');
    }
}
unset($ot);

// Debug: Log detailed order totals information
log_paypalac_wallet_message('Order totals calculated in base currency (' . $baseCurrency . '):');
foreach ($order_totals as $index => $ot) {
    log_paypalac_wallet_message('  Order total ' . ($index + 1) . ': ' . print_r([
        'code' => $ot['code'] ?? 'N/A',
        'title' => $ot['title'] ?? 'N/A',
        'text' => $ot['text'] ?? 'N/A',
        'value' => $ot['value'] ?? 'N/A',
...
log_paypalac_wallet_message('Order info after totals: subtotal=' . $order->info['subtotal'] . ', currency=' . $baseCurrency . ', shipping_cost=' . ($order->info['shipping_cost'] ?? 'N/A') . ', tax=' . ($order->info['tax'] ?? 'N/A') . ', total=' . ($order->info['total'] ?? 'N/A'));
...
log_paypalac_wallet_message('Starting total calculation with subtotal in base currency (' . $baseCurrency . '): ' . $calculatedTotalBase);

foreach ($order_totals as $ot) {
    $type = 'LINE_ITEM';
    $valueBase = $ot['value']; // Value in base currency
    log_paypalac_wallet_message('Processing order total: code=' . $ot['code'] . ', base value=' . $valueBase . ', title=' . $ot['title']);
    ...
    if ($ot['code'] === 'ot_subtotal') {
        ...
        log_paypalac_wallet_message('Added shipping to total (base): ' . $valueBase . ', new total (base): ' . $calculatedTotalBase);
        ...
        $calculatedTotalBase += $valueBase;
        log_paypalac_wallet_message('Added discount to total (base): ' . $valueBase . ', new total (base): ' . $calculatedTotalBase);
    } elseif ($ot['code'] === 'ot_total') {
        // ignore built-in total; we're computing it manually
        log_paypalac_wallet_message('Skipping ot_total (calculated manually)');
        continue;
    } else {
        $calculatedTotalBase += $valueBase;
        log_paypalac_wallet_message('Added other order total to total (base): ' . $valueBase . ', new total (base): ' . $calculatedTotalBase);
    }
...
if ($totalTaxBase > 0) {
    ...
    log_paypalac_wallet_message('Added combined tax display item (base): ' . $totalTaxBase . ', display: ' . $totalTaxDisplay);
}

if (!$shippingAddedToTotal) {
    $calculatedTotalBase += $_SESSION['shipping']['cost'] ?? 0;
    log_paypalac_wallet_message('Shipping was not in order_totals, adding from session (base): ' . ($_SESSION['shipping']['cost'] ?? 0) . ', new total (base): ' . $calculatedTotalBase);
}
...
log_paypalac_wallet_message('Currency conversion summary:');
log_paypalac_wallet_message('  Base currency: ' . $baseCurrency);
log_paypalac_wallet_message('  Display currency: ' . $displayCurrency);
log_paypalac_wallet_message('  Base currency rate: ' . ($currencies->currencies[$baseCurrency]['value'] ?? 1));
log_paypalac_wallet_message('  Display currency rate: ' . $displayCurrencyValue);
log_paypalac_wallet_message('  Total in base currency (' . $baseCurrency . '): ' . $calculatedTotalBase);
log_paypalac_wallet_message('  Total in display currency (' . $displayCurrency . '): ' . $calculatedTotalDisplay);
log_paypalac_wallet_message('Final total formatted for payment: ' . $finalTotal);
log_paypalac_wallet_message('Display items: ' . print_r($displayItems, true));
