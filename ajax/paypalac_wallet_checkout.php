...
if ($incomingSessionParam !== null && !empty($_GET[$incomingSessionParam])) {
    ...
    if (!isset($_COOKIE['cookie_test'])) {
        $_COOKIE['cookie_test'] = 'paypalac_wallet_session_bridge';
    }
}
...
if (function_exists('zen_load_language')) {
    ...
    paypalac_wallet_load_language_file('checkout_process');
    paypalac_wallet_load_language_file('email_extras');
}
...
$braintreeCheckoutErrorResponder = function ($message) use (&$braintreeCheckoutErrorHandled) {
    ...
    paypalac_wallet_checkout_log($message);
...
log_paypalac_wallet_message('Received checkout payload: ' . print_r($payload, true));
...
$module = $payload['module']; // Payment module (e.g., paypalac_googlepay, paypalac_applepay, etc.)
...
log_paypalac_wallet_message("PayPal order ID: " . ($paypal_order_id ?: 'N/A'));
...
if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    log_paypalac_wallet_message("ERROR: Invalid or missing email address: " . var_export($email, true));
    echo json_encode([
        'status' => 'error',
...
log_paypalac_wallet_message("Processing order for email: $email");
...
if ((!$shipping_complete && $shipping_required) || !$billing_complete) {
    global $messageStack;
    log_paypalac_wallet_message('Raw shipping address: ' . print_r($shipping_address_raw, true));
    log_paypalac_wallet_message('Raw billing address: ' . print_r($billing_address_raw, true));
    if (!$shipping_complete && $shipping_required) {
        log_paypalac_wallet_message('Missing shipping fields: ' . implode(', ', $shipping_errors));
    }
    if (!$billing_complete) {
        log_paypalac_wallet_message('Missing billing fields: ' . implode(', ', $billing_errors));
    }
    log_paypalac_wallet_message('Incomplete shipping or billing address. Redirecting to checkout.');
...
if ($shipping_required && empty($_SESSION['shipping']['id'])) {
    global $messageStack;
    log_paypalac_wallet_message('Missing shipping selection. Redirecting to checkout.');
    $messageStack->add_session('header', 'Your payment was not processed. Please complete checkout.', 'error');
    echo json_encode([
        'status' => 'success',
        'redirect_url' => zen_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL')
...
log_paypalac_wallet_message("Using payment nonce: $payment_method_nonce");
log_paypalac_wallet_message("Billing address: " . print_r($billing_address, true));
log_paypalac_wallet_message("Shipping address: " . print_r($shipping_address, true));

// Validate module
$valid_modules = ['braintree_api', 'paypalac_googlepay', 'paypalac_applepay', 'paypalac_venmo'];
if (!in_array($module, $valid_modules)) {
    die('Invalid payment module');
}

// Determine if guest flow should be used
$guestModules = ['paypalac_googlepay', 'paypalac_applepay'];
...
if (empty($_SESSION['customer_id'])) {
    if ($customer_query->RecordCount() > 0) {
        ...
        log_paypalac_wallet_message("Using existing customer ID: $customer_id");
    } else {
        ...
        log_paypalac_wallet_message("Created new customer ID: $customer_id with email: $email");
    ...
    if ($isGuestCheckout) {
        ...
        if (in_array($module, ['paypalac_googlepay', 'paypalac_applepay'], true)) {
...
log_paypalac_wallet_message("Shipping address lookup: administrativeArea='" . ($shipping_address['administrativeArea'] ?? '') . "', countryCode='" . ($shipping_address['countryCode'] ?? '') . "', resolved zone_id=$shipping_zone_id, zone_name='$shipping_zone_name'");
...
if ($address_query->RecordCount() > 0) {
    ...
    log_paypalac_wallet_message("Found existing shipping address ID $address_id: existing zone_id=$existing_zone_id, existing state='$existing_state'");
    ...
    if ($existing_zone_id !== $shipping_zone_id) {
        ...
        log_paypalac_wallet_message("Updating shipping address ID $address_id: changing zone_id from $existing_zone_id to $shipping_zone_id, state to '$entry_state'");
    ...
    log_paypalac_wallet_message("Creating shipping address: administrativeArea='" . ($shipping_address['administrativeArea'] ?? '') . "', countryCode='" . ($shipping_address['countryCode'] ?? '') . "', zone_id=$shipping_zone_id, zone_name='$shipping_zone_name', entry_state='$entry_state'");
...
log_paypalac_wallet_message("Billing address lookup: administrativeArea='" . ($billing_address['administrativeArea'] ?? '') . "', countryCode='" . ($billing_address['countryCode'] ?? '') . "', resolved zone_id=$billing_zone_id, zone_name='$billing_zone_name'");
...
if ($billing_address_query->RecordCount() > 0) {
    ...
    log_paypalac_wallet_message("Found existing billing address ID $billing_address_id: existing zone_id=$existing_billing_zone_id, existing state='$existing_billing_state'");
    ...
    if ($existing_billing_zone_id !== $billing_zone_id) {
        ...
        log_paypalac_wallet_message("Updating billing address ID $billing_address_id: changing zone_id from $existing_billing_zone_id to $billing_zone_id, state to '$billing_entry_state'");
    ...
    log_paypalac_wallet_message("Creating billing address: administrativeArea='" . ($billing_address['administrativeArea'] ?? '') . "', countryCode='" . ($billing_address['countryCode'] ?? '') . "', zone_id=$billing_zone_id, zone_name='$billing_zone_name', entry_state='$billing_entry_state'");
...
log_paypalac_wallet_message("Billing address ID: $billing_address_id");
log_paypalac_wallet_message("Shipping address ID: $address_id");
...
if (
    ...
    defined('MODULE_PAYMENT_PAYPALAC_APPLE_PAY_CONFIRM_REDIRECT') &&
    MODULE_PAYMENT_PAYPALAC_APPLE_PAY_CONFIRM_REDIRECT === 'True'
) {
    // set payment info for the upcoming confirmation page
    $_SESSION['payment'] = $_POST['payment'] = 'paypalac_applepay';
...
if (isset($order->delivery['zone_id']) && $order->delivery['zone_id'] > 0) {
    ...
    if ($delivery_zone_name !== '') {
        ...
        log_paypalac_wallet_message("Fixed delivery state: zone_id=" . $order->delivery['zone_id'] . ", state set to '" . $delivery_zone_name . "'");
    }
}
...
if (isset($order->billing['zone_id']) && $order->billing['zone_id'] > 0) {
    ...
    if ($billing_zone_name !== '') {
        ...
        log_paypalac_wallet_message("Fixed billing state: zone_id=" . $order->billing['zone_id'] . ", state set to '" . $billing_zone_name . "'");
    }
}
