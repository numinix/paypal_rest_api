<?php
/**
 * PayPal Advanced Checkout (paypalac) partner integrated sign-up.
 *
...
if (function_exists('zen_admin_check_login')) {
    $paypalacAdminLoggedIn = zen_admin_check_login();
} else {
    $paypalacAdminLoggedIn = (int)($_SESSION['admin_id'] ?? 0) > 0;
}
...
if (!$paypalacAdminLoggedIn) {
    // For AJAX requests, return JSON error instead of HTML redirect
    if ($isAjaxRequest) {
        paypalac_json_error('Session expired. Please refresh the page and log in again.');
    }
...
if ($action === 'proxy' && $isAjaxRequest) {
    paypalac_handle_proxy_request();
    exit;
}

if ($action === 'save_credentials' && $isAjaxRequest) {
    paypalac_handle_save_credentials();
    exit;
}

// Handle JavaScript SDK credential save - Phase 2
if ($action === 'save_isu_credentials' && $isAjaxRequest) {
    paypalac_handle_save_isu_credentials();
    exit;
}

// Handle completion flow - receives PayPal redirect with credentials
if ($action === 'complete') {
    paypalac_handle_completion();
    exit;
}

// Handle legacy redirect-based flow for backwards compatibility
if ($action === 'return') {
    paypalac_handle_legacy_return();
    exit;
}

if ($action === 'cancel') {
    paypalac_onboarding_message(
        MODULE_PAYMENT_PAYPALAC_TEXT_ADMIN_ISU_CANCEL_MESSAGE ?? 'PayPal onboarding was cancelled before completion. You can restart the process at any time.',
        'warning'
    );
    paypalac_redirect_to_modules();
    exit;
}

// Default: Show the in-admin onboarding page
paypalac_render_onboarding_page();
exit;

/**
 * Proxies AJAX requests to the Numinix.com API.
 */
function paypalac_handle_proxy_request(): void
{
    header('Content-Type: application/json');

    $proxyAction = strtolower(trim((string)($_REQUEST['proxy_action'] ?? '')));
    $numinixUrl = paypalac_get_numinix_portal_base();
    $storeEnvironment = paypalac_detect_environment();

    paypalac_log_debug('Handling proxy request', [
    ...
    if ($numinixUrl === '') {
        paypalac_json_error('Numinix portal URL not configured.');
        return;
    }
    ...
    if (!in_array($proxyAction, $allowedActions, true)) {
        paypalac_json_error('Invalid proxy action.');
        return;
    }
    
    try {
        $response = paypalac_proxy_to_numinix($numinixUrl, $proxyAction, $_POST);
        ...
        if ($proxyAction === 'start' && !empty($data['nonce'])) {
            $_SESSION['paypalac_isu_nonce'] = $data['nonce'];
            paypalac_log_debug('Stored nonce in session for completion page', [
                'tracking_id' => $data['tracking_id'] ?? null,
            ]);
        }
        ...
        if ($proxyAction === 'start' && !empty($data['seller_nonce'])) {
            $_SESSION['paypalac_isu_seller_nonce'] = $data['seller_nonce'];
            paypalac_log_debug('Stored seller_nonce in session for credential exchange', [
                'tracking_id' => $data['tracking_id'] ?? null,
            ]);
        }
        ...
        if (in_array($proxyAction, ['status', 'finalize'], true)) {
            $logContext['sanitized_data'] = paypalac_redact_sensitive($data);
        }

        paypalac_log_debug('Proxy request completed', $logContext);
        
        echo $response;
    } catch (Exception $e) {
        paypalac_log_debug('Proxy request failed with exception', [
            'proxy_action' => $proxyAction,
            'exception' => get_class($e),
            'message' => $e->getMessage(),
        ]);
        paypalac_json_error($e->getMessage());
    }
}
 ...
 */
function paypalac_proxy_to_numinix(string $baseUrl, string $action, array $data): string
{
    $url = rtrim($baseUrl, '/');
    [$environment, $environmentSource] = paypalac_resolve_environment_from_request($data);
    ...
    $origin = paypalac_get_origin();
    
    $postData = http_build_query($payload, '', '&', PHP_QUERY_RFC3986);
    
    paypalac_log_debug('Proxy request to Numinix API', [
        'action' => $action,
        'url' => $url,
    ...
    if ($response === false) {
        ...
        paypalac_log_debug('Numinix API request failed', [
            'action' => $action,
    ...
    if ($httpCode !== 200) {
        ...
        paypalac_log_debug('Numinix API returned non-200 status', [
            'action' => $action,
            'http_code' => $httpCode,
    ...
    paypalac_log_debug('Numinix API response received', [
        'action' => $action,
        'http_code' => $httpCode,
 ...
 */
function paypalac_handle_legacy_return(): void
{
    $client_id = trim((string)($_GET['client_id'] ?? ''));
    $client_secret = trim((string)($_GET['client_secret'] ?? ''));
    $environment = paypalac_detect_environment();
    
    $credentials_saved = false;
    if ($client_id !== '' && $client_secret !== '') {
        $credentials_saved = paypalac_save_credentials($client_id, $client_secret, $environment);
    }
    
    if ($credentials_saved) {
        paypalac_onboarding_message(
            MODULE_PAYMENT_PAYPALAC_TEXT_ADMIN_ISU_SUCCESS_AUTO ?? 'PayPal onboarding completed successfully! Your API credentials have been automatically configured.',
            'success'
        );
    } else {
        paypalac_onboarding_message(
 ...
 */
function paypalac_render_onboarding_page(): void
{
    $environment = paypalac_detect_environment();
    $modulesPageUrl = paypalac_modules_page_url();
    $proxyUrl = paypalac_self_url(['action' => 'proxy']);
    $saveUrl = paypalac_self_url(['action' => 'save_credentials']);
    $completeUrl = paypalac_self_url(['action' => 'complete']);
    ...
    <body>
        ...
        <script src="includes/javascript/paypalac_integrated_signup_callback.js"></script>
        
        <script>
            window.paypalacISUConfig = {
        ...
        <script src="includes/javascript/paypalac_integrated_signup.js"></script>
    </body>
    </html>
    <?php
}

function paypalac_json_error(string $message): void
{
    paypalac_log_debug('Returning JSON error to client', ['message' => $message]);
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => $message]);
    exit;
}

function paypalac_onboarding_message(string $message, string $type = 'warning'): void
{
    global $messageStack;
...
function paypalac_redirect_to_modules(): void
{
    zen_redirect(paypalac_modules_page_url());
    exit;
}

function paypalac_modules_page_url(): string
{
    return html_entity_decode(
        zen_href_link(FILENAME_MODULES, 'set=payment&module=paypalac', 'SSL', false),
        ENT_QUOTES,
        'UTF-8'
    );
}

function paypalac_get_numinix_portal_base(): string
{
    ...
    if (defined('MODULE_PAYMENT_PAYPALAC_NUMINIX_PORTAL') && MODULE_PAYMENT_PAYPALAC_NUMINIX_PORTAL !== '') {
        $baseUrl = trim((string)MODULE_PAYMENT_PAYPALAC_NUMINIX_PORTAL);
    }
 ...
 */
function paypalac_resolve_environment_from_request(array $data): array
{
    ...
    return [paypalac_detect_environment(), 'config'];
}

function paypalac_detect_environment(): string
{
    $allowed = ['sandbox', 'live'];
...
function paypalac_self_url(array $params = []): string
{
...
function paypalac_save_credentials(string $client_id, string $client_secret, string $environment): bool
{
    global $db;
    ...
    if ($environment === 'live') {
        $client_id_key = 'MODULE_PAYMENT_PAYPALAC_CLIENTID_L';
        $client_secret_key = 'MODULE_PAYMENT_PAYPALAC_SECRET_L';
    } else {
        $client_id_key = 'MODULE_PAYMENT_PAYPALAC_CLIENTID_S';
        $client_secret_key = 'MODULE_PAYMENT_PAYPALAC_SECRET_S';
    }
 ...
 */
function paypalac_handle_save_credentials(): void
{
    header('Content-Type: application/json');

    $token = (string)($_POST['securityToken'] ?? '');
    if ($token === '' || $token !== (string)($_SESSION['securityToken'] ?? '')) {
        paypalac_json_error('Invalid security token. Please refresh and try again.');
    }
    ...
    if ($clientId === '' || $clientSecret === '') {
        paypalac_json_error('Client ID and Secret are required.');
    }
    ...
    $saved = paypalac_save_credentials($clientId, $clientSecret, $environment);

    if (!$saved) {
        paypalac_json_error('Unable to save PayPal credentials.');
    }

    paypalac_log_debug('PayPal ISU credentials saved', [
        'environment' => $environment,
 ...
 */
function paypalac_handle_save_isu_credentials(): void
{
    header('Content-Type: application/json');
    ...
    if ($merchantId === '') {
        paypalac_json_error('Merchant ID is required.');
    }
    ...
    paypalac_log_debug('JavaScript SDK credential save initiated', [
    ...
    paypalac_log_debug('Proxying to Numinix finalize API', [
        'url' => $numinixUrl,
        'environment' => $environment,
        'has_auth_code' => $authCode !== '',
        'has_shared_id' => $sharedId !== '',
    ...
    if ($response === false || $curlError !== '') {
        ...
        paypalac_json_error('Failed to contact Numinix API: ' . $curlError);
    }
    ...
    if (!is_array($data)) {
        paypalac_log_debug('Invalid JSON response from Numinix API', [
            'response' => substr((string)$response, 0, 500),
        ]);
        paypalac_json_error('Invalid response from Numinix API.');
    }

    // Check for API errors
    if (!isset($data['success']) || $data['success'] !== true) {
        $errorMessage = $data['message'] ?? 'Unknown error from Numinix API';
        paypalac_log_debug('Numinix API returned error', [
            'message' => $errorMessage,
            'data' => $data,
        ]);
        paypalac_json_error($errorMessage);
    }
    ...
    if ($clientId === '' || $clientSecret === '') {
        ...
        paypalac_log_debug('Credentials not yet available', [
            'step' => $step,
    ...
    if (!$saved) {
        paypalac_log_debug('Failed to save credentials to database');
        paypalac_json_error('Unable to save PayPal credentials to database.');
    }

    paypalac_log_debug('JavaScript SDK ISU credentials saved successfully', [
        'environment' => $environment,
 ...
 * @return string
 */
function paypalac_get_origin(): string
{
    $scheme = 'http';
    if (!empty($_SERVER['HTTPS']) && strtolower((string) $_SERVER['HTTPS']) !== 'off') {
        $scheme = 'https';
 ...
 * @return void
 */
function paypalac_log_debug(string $message, array $context = []): void
{
    $logFile = paypalac_resolve_log_file();
    if ($logFile === null) {
        return;
    }
    ...
    if (!empty($context)) {
        ...
        $sanitized = paypalac_redact_sensitive($context);
 ...
 * @return string|null
 */
function paypalac_resolve_log_file(): ?string
{
    $baseDir = null;
    ...
    return $baseDir . '/paypalac_isu_debug.log';
}
 ...
 * @return mixed
 */
function paypalac_redact_sensitive($value)
{
    ...
    if (is_array($value)) {
        ...
        foreach ($value as $key => $item) {
            ...
            $redacted[$key] = paypalac_redact_sensitive($item);
        }
        return $redacted;
    }
 ...
 */
function paypalac_handle_completion(): void
{
    $proxyUrl = paypalac_self_url(['action' => 'proxy']);
    $saveUrl = paypalac_self_url(['action' => 'save_credentials']);
    $modulesPageUrl = paypalac_modules_page_url();
    ...
    $trackingId = trim((string)($_GET['tracking_id'] ?? $_SESSION['paypalac_isu_tracking_id'] ?? ''));
    $environment = trim((string)($_GET['env'] ?? $_SESSION['paypalac_isu_environment'] ?? paypalac_detect_environment()));
    ...
    paypalac_log_debug('Completion handler called', [
        'has_merchant_id' => $merchantId !== '',
        'has_auth_code' => $authCode !== '',
        'has_shared_id' => $sharedId !== '',
    ...
    $nonce = trim((string)($_SESSION['paypalac_isu_nonce'] ?? ''));
    
    // Retrieve seller_nonce from session (needed for code_verifier in credential exchange)
    $sellerNonce = trim((string)($_SESSION['paypalac_isu_seller_nonce'] ?? ''));
    
    // Store in session for retrieval
    if ($merchantId !== '') {
        $_SESSION['paypalac_isu_merchant_id'] = $merchantId;
    }
    if ($authCode !== '') {
        $_SESSION['paypalac_isu_auth_code'] = $authCode;
    }
    if ($sharedId !== '') {
        $_SESSION['paypalac_isu_shared_id'] = $sharedId;
    }
    if ($trackingId !== '') {
        $_SESSION['paypalac_isu_tracking_id'] = $trackingId;
    }
    $_SESSION['paypalac_isu_environment'] = $environment;
    ...
    <body>
        ...
        <script>
            window.paypalacISUCompleteConfig = {
                proxyUrl: <?php echo json_encode($proxyUrl); ?>,
                saveUrl: <?php echo json_encode($saveUrl); ?>,
                modulesPageUrl: <?php echo json_encode($modulesPageUrl); ?>,
                securityToken: <?php echo json_encode($securityToken); ?>,
        ...
        <script src="includes/javascript/paypalac_integrated_signup_complete.js"></script>
    </body>
    </html>
    <?php
}
