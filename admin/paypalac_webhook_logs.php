...
if ($action === 'clear' && isset($_POST['confirm_clear']) && $_POST['confirm_clear'] === 'yes') {
    ...
    zen_redirect(zen_href_link(FILENAME_PAYPALAC_WEBHOOK_LOGS));
}
...
function whl_page_url(int $pageNum, string $webhookId = '', string $eventType = '', string $search = ''): string
{
    ...
    return zen_href_link(FILENAME_PAYPALAC_WEBHOOK_LOGS, $params);
}
        ...
        <div class="nmx-panel">
            ...
            <div class="nmx-panel-body">
                ...
                <div style="margin-top: 24px;">
                    <a href="<?php echo zen_href_link(FILENAME_PAYPALAC_WEBHOOK_LOGS, zen_get_all_get_params(['detail', 'action'])); ?>" class="nmx-btn nmx-btn-default"><?php echo TEXT_BUTTON_BACK; ?></a>
                </div>
            </div>
        </div>
...
<?php } else { ?>
        ...
        <div class="nmx-panel">
            ...
            <div class="nmx-panel-body">
                <?php echo zen_draw_form('paypalac_webhook_search', FILENAME_PAYPALAC_WEBHOOK_LOGS, '', 'get', 'class="nmx-form-inline"'); ?>
                    ...
                    <div class="nmx-form-actions">
                        ...
                        <a href="<?php echo zen_href_link(FILENAME_PAYPALAC_WEBHOOK_LOGS); ?>" class="nmx-btn nmx-btn-default"><?php echo TEXT_BUTTON_RESET; ?></a>
                    </div>
                </form>
            </div>
        </div>
                            ...
                            <tbody>
                                <?php foreach ($logs as $log) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?php echo zen_href_link(FILENAME_PAYPALAC_WEBHOOK_LOGS, 'detail=' . (int)$log['id'] . '&' . zen_get_all_get_params(['detail', 'action'])); ?>" class="nmx-btn nmx-btn-sm nmx-btn-info"><?php echo TEXT_VIEW_DETAILS; ?></a>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
        ...
        <?php if ($totalRecords > 0) { ?>
            <div class="nmx-panel">
                <div class="nmx-panel-body">
                    <?php echo zen_draw_form('paypalac_webhook_clear', FILENAME_PAYPALAC_WEBHOOK_LOGS, 'action=clear', 'post', 'onsubmit="return confirm(\'' . htmlspecialchars(TEXT_CONFIRM_CLEAR, ENT_QUOTES, 'UTF-8') . '\');"'); ?>
