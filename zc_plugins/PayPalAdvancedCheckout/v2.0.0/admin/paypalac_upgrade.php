<?php
/**
 * PayPal Advanced Checkout (paypalac) module upgrade handler.
 *
...
$valid_modules = ['paypalac', 'paypalac_applepay', 'paypalac_googlepay', 'paypalac_venmo', 'paypalac_savedcard'];
if (!in_array($module_code, $valid_modules, true)) {
    paypalac_upgrade_message(
        'Invalid module specified for upgrade.',
        'error'
    );
    paypalac_upgrade_redirect_to_modules();
}

if ($action === 'upgrade') {
    // Load the payment module
    $module_file = dirname(__DIR__) . '/catalog/includes/modules/payment/' . $module_code . '.php';
...
function paypalac_upgrade_message(string $message, string $type = 'warning'): void
{
    global $messageStack;
...
function paypalac_upgrade_redirect_to_modules(): void
{
    zen_redirect(zen_href_link(FILENAME_MODULES, 'set=payment&module=' . ($_GET['module'] ?? 'paypalac'), 'NONSSL'));
    exit;
}

// In case we ever need to render an upgrade confirmation page (not currently used)
function paypalac_upgrade_render_page(): void
{
    ?>
    <!doctype html>
