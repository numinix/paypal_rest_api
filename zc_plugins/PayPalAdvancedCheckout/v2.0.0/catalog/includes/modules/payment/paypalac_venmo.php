<?php
/**
 * paypalac_venmo.php payment module class for handling Venmo via PayPal Advanced Checkout.
 *
...
LanguageCompatibility::load('paypalac_venmo');

/**
 * The PayPal Venmo payment module using PayPal's REST APIs (v2)
 */
class paypalac_venmo extends base
{
    protected function getModuleStatusSetting(): string
    {
        return defined('MODULE_PAYMENT_PAYPALAC_VENMO_STATUS') ? MODULE_PAYMENT_PAYPALAC_VENMO_STATUS : 'False';
    }

    protected function getModuleSortOrder(): ?int
    {
        return defined('MODULE_PAYMENT_PAYPALAC_VENMO_SORT_ORDER') ? (int)MODULE_PAYMENT_PAYPALAC_VENMO_SORT_ORDER : null;
    }

    protected function getModuleZoneSetting(): int
    {
        return defined('MODULE_PAYMENT_PAYPALAC_VENMO_ZONE') ? (int)MODULE_PAYMENT_PAYPALAC_VENMO_ZONE : 0;
    }
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            $this->title = defined('MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE')
                ? MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE
                : 'PayPal Venmo';
        } else {
            $adminTitle = defined('MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE_ADMIN')
                ? MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE_ADMIN
                : 'PayPal Venmo';
            ...
            $descriptionTemplate = defined('MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_DESCRIPTION')
                ? MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_DESCRIPTION
                : 'Venmo via PayPal Advanced Checkout (v%s)';
            ...
            $installed_version = defined('MODULE_PAYMENT_PAYPALAC_VENMO_VERSION') ? MODULE_PAYMENT_PAYPALAC_VENMO_VERSION : '0.0.0';
            if ($installed_version !== '0.0.0' && version_compare($installed_version, self::CURRENT_VERSION, '<')) {
                ...
                $this->description .= sprintf(
                    ...
                    zen_href_link('paypalac_upgrade.php', 'module=paypalac_venmo&action=upgrade', 'SSL')
                );
            }
        }
        ...
        // is to be performed. Venmo is a wallet payment (ppac_type !== 'card').
        //
        ...
        if (!defined('MODULE_PAYMENT_PAYPALAC_VERSION')) {
            $this->enabled = false;
            if (IS_ADMIN_FLAG === true) {
                $this->title .= $this->alertMsg(MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_PAYPAL_REQUIRED ?? 'Main PayPal module required');
            }
            return;
        }
    }
    ...
    protected function tableCheckup()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALAC_VENMO_VERSION') && MODULE_PAYMENT_PAYPALAC_VENMO_VERSION === $current_version) {
            return;
        }
        
        // Check for version-specific configuration updates
        if (defined('MODULE_PAYMENT_PAYPALAC_VENMO_VERSION')) {
            switch (true) {
                case version_compare(MODULE_PAYMENT_PAYPALAC_VENMO_VERSION, '1.3.4', '<'):
                    $account_id_setting = $db->Execute(
                        "SELECT configuration_id
                           FROM " . TABLE_CONFIGURATION . "
                          WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID'
                          LIMIT 1"
                    );
                    if (!defined('MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID') && $account_id_setting->EOF) {
                        $db->Execute(
                            "INSERT INTO " . TABLE_CONFIGURATION . "
                                (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
                             VALUES
                                ('Venmo Business Profile ID', 'MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID', '', 'Enter the Venmo business profile or account ID tied to your PayPal Advanced Checkout credentials. This value is required for Venmo transactions.', 6, 0, NULL, NULL, now())"
                        );
                    }
                    // Fall through to update the stored version number

                case version_compare(MODULE_PAYMENT_PAYPALAC_VENMO_VERSION, '1.3.5', '<'):
                    $db->Execute(
                        "UPDATE " . TABLE_CONFIGURATION . "
                            SET configuration_description = 'Enter the Venmo business profile or account ID tied to your PayPal Advanced Checkout credentials. This value is required for Venmo transactions.'
                          WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID'
                          LIMIT 1"
                    );
                    // Fall through to update the stored version number

                case version_compare(MODULE_PAYMENT_PAYPALAC_VENMO_VERSION, '1.3.6', '<'):
                    $db->Execute(
                        "UPDATE " . TABLE_CONFIGURATION . "
                ...
                case version_compare(MODULE_PAYMENT_PAYPALAC_VENMO_VERSION, '1.3.7', '<'):
                    // Add configuration options to control button display on cart and product pages
                    $db->Execute(
                        "INSERT IGNORE INTO " . TABLE_CONFIGURATION . "
                            (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
                         VALUES
                            ('Enable on Shopping Cart Page?', 'MODULE_PAYMENT_PAYPALAC_VENMO_SHOPPING_CART', 'True', 'Do you want to display the Venmo button on the shopping cart page?', 6, 0, 'zen_cfg_select_option([''True'', ''False''], ', NULL, now()),
                            ('Enable on Product Page?', 'MODULE_PAYMENT_PAYPALAC_VENMO_PRODUCT_PAGE', 'True', 'Do you want to display the Venmo button on product pages?', 6, 0, 'zen_cfg_select_option([''True'', ''False''], ', NULL, now())"
                    );
                    // Fall through to update the stored version number
        ...
        $db->Execute(
            "UPDATE " . TABLE_CONFIGURATION . "
                SET configuration_value = '$current_version',
                    last_modified = now()
              WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_VENMO_VERSION'
              LIMIT 1"
        );
    }
    ...
    protected function getPayPalAdvancedCheckoutApi(): ?PayPalAdvancedCheckoutApi
    {
        ...
        try {
            ...
            $this->log->write('Venmo: Error creating PayPalAdvancedCheckoutApi: ' . $e->getMessage());
    ...
    public function update_status()
    {
        ...
        // Disable Venmo for subscription products - Venmo does not support vaulting for recurring payments
     ...
     * 
     * Venmo does not support vaulting for recurring payments, so this module
    ...
    public function selection(): array
    {
        ...
        $buttonContainer = '<div id="paypalac-venmo-button" class="paypalac-venmo-button"></div>';
        $hiddenFields =
            zen_draw_hidden_field('ppac_type', 'venmo') .
            zen_draw_hidden_field('paypalac_venmo_payload', '', 'id="paypalac-venmo-payload"') .
            zen_draw_hidden_field('paypalac_venmo_status', '', 'id="paypalac-venmo-status"');
    ...
    public function ajaxGetWalletConfig(): array
    {
        ...
        $merchant_id = defined('MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID') ? trim((string)MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID) : '';
        ...
        if ($client_id === '') {
            ...
            return ['success' => false, 'message' => MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_INITIALIZE ?? 'Unable to start Venmo. Please try again.'];
        }
    ...
    public function ajaxCreateWalletOrder(): array
    {
        ...
        if ($response['success'] === false && empty($response['message'])) {
            $response['message'] = MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_INITIALIZE ?? 'Unable to start Venmo. Please try again.';
        }
    ...
    public function pre_confirmation_check()
    {
        $this->paypalCommon->processWalletConfirmation(
            'venmo',
            'paypalac_venmo_payload',
            [
                'title' => MODULE_PAYMENT_PAYPALAC_VENMO_TEXT_TITLE ?? 'Venmo',
                'payload_missing' => MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_PAYLOAD_MISSING ?? 'Payload missing',
                'payload_invalid' => MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_PAYLOAD_INVALID ?? 'Invalid payload',
                'confirm_failed' => MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_CONFIRM_FAILED ?? 'Confirmation failed',
                'payer_action' => MODULE_PAYMENT_PAYPALAC_VENMO_ERROR_PAYER_ACTION ?? 'Payer action required',
            ]
        );
    }
    ...
    public function before_process()
    {
        ...
        if (in_array($payment['status'], ['DECLINED', 'DENIED', 'FAILED'], true)) {
            $this->log->write("==> Venmo::before_process: Payment {$payment['status']}; redirecting to checkout.");
            unset($_SESSION['PayPalAdvancedCheckout']['Order'], $_SESSION['payment']);
    ...
    protected function recordPayPalOrderDetails(int $orders_id): void
    {
        ...
        // Implementation similar to paypalac but adapted for Venmo
        if ($orders_id <= 0) {
            return;
        }
    ...
    public function check(): bool
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_VENMO_STATUS'");
    ...
    public function install()
    {
        ...
        $db->Execute(
            "INSERT INTO " . TABLE_CONFIGURATION . "
                (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
             VALUES
                ('Module Version', 'MODULE_PAYMENT_PAYPALAC_VENMO_VERSION', '$current_version', 'Currently-installed module version.', 6, 0, 'zen_cfg_read_only(', NULL, now()),
                ('Enable PayPal Venmo?', 'MODULE_PAYMENT_PAYPALAC_VENMO_STATUS', 'False', 'Do you want to enable PayPal Venmo payments?', 6, 0, 'zen_cfg_select_option([''True'', ''False'', ''Retired''], ', NULL, now()),
                ('Venmo Business Profile ID', 'MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID', '', 'Optional Venmo business profile ID reserved for future Venmo buttons; not required for PayPal-managed Venmo checkout.', 6, 0, NULL, NULL, now()),
                ('Sort order of display.', 'MODULE_PAYMENT_PAYPALAC_VENMO_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', 6, 0, NULL, NULL, now()),
                ('Payment Zone', 'MODULE_PAYMENT_PAYPALAC_VENMO_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', 6, 0, 'zen_cfg_pull_down_zone_classes(', 'zen_get_zone_class_title', now())" 
        );
        
        // -----
        // Migrate configuration values from the old paypalr_venmo module if it was previously installed.
        ...
        $old_config = $db->Execute(
            "SELECT configuration_key, configuration_value
               FROM " . TABLE_CONFIGURATION . "
              WHERE configuration_key LIKE 'MODULE\_PAYMENT\_PAYPALR\_VENMO\_%'"
        );
        while (!$old_config->EOF) {
            $old_key = $old_config->fields['configuration_key'];
            $new_key = str_replace('MODULE_PAYMENT_PAYPALR_VENMO_', 'MODULE_PAYMENT_PAYPALAC_VENMO_', $old_key);
        ...
        define('MODULE_PAYMENT_PAYPALAC_VENMO_VERSION', '0.0.0');
        $this->tableCheckup();
    }
    ...
    {
        return [
            'MODULE_PAYMENT_PAYPALAC_VENMO_VERSION',
            'MODULE_PAYMENT_PAYPALAC_VENMO_STATUS',
            'MODULE_PAYMENT_PAYPALAC_VENMO_ACCOUNT_ID',
            'MODULE_PAYMENT_PAYPALAC_VENMO_SORT_ORDER',
            'MODULE_PAYMENT_PAYPALAC_VENMO_ZONE',
            'MODULE_PAYMENT_PAYPALAC_VENMO_SHOPPING_CART',
            'MODULE_PAYMENT_PAYPALAC_VENMO_PRODUCT_PAGE',
        ];
    }
    ...
    {
        global $db;
        $db->Execute("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key LIKE 'MODULE_PAYMENT_PAYPALAC_VENMO_%'");
    }
