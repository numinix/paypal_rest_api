<?php
/**
 * paypalac_googlepay.php payment module class for handling Google Pay via PayPal Advanced Checkout.
 *
...
LanguageCompatibility::load('paypalac_googlepay');

/**
 * The PayPal Google Pay payment module using PayPal's REST APIs (v2)
 */
class paypalac_googlepay extends base
{
    protected function getModuleStatusSetting(): string
    {
        return defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_STATUS') ? MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_STATUS : 'False';
    }

    protected function getModuleSortOrder(): ?int
    {
        return defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SORT_ORDER') ? (int)MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SORT_ORDER : null;
    }

    protected function getModuleZoneSetting(): int
    {
        return defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ZONE') ? (int)MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ZONE : 0;
    }
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            $this->title = defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE')
                ? MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE
                : 'PayPal Google Pay';
        } else {
            $adminTitle = defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE_ADMIN')
                ? MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE_ADMIN
                : 'PayPal Google Pay';
            ...
            $descriptionTemplate = defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_DESCRIPTION')
                ? MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_DESCRIPTION
                : 'Google Pay via PayPal Advanced Checkout (v%s)';
            ...
            $installed_version = defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION') ? MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION : '0.0.0';
            if ($installed_version !== '0.0.0' && version_compare($installed_version, self::CURRENT_VERSION, '<')) {
                ...
                $this->description .= sprintf(
                    ...
                    zen_href_link('paypalac_upgrade.php', 'module=paypalac_googlepay&action=upgrade', 'SSL')
                );
            }
        }
        ...
        // is to be performed. Google Pay is a wallet payment (ppac_type !== 'card').
        //
        ...
        if (!defined('MODULE_PAYMENT_PAYPALAC_VERSION')) {
            $this->enabled = false;
            if (IS_ADMIN_FLAG === true) {
                $this->title .= $this->alertMsg(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ERROR_PAYPAL_REQUIRED ?? 'Main PayPal module required');
            }
            return;
        }
    }
    ...
    protected function tableCheckup()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION') && MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION === $current_version) {
            return;
        }
        
        // Check for version-specific configuration updates
        if (defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION')) {
            switch (true) {
                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.6', '<'):
                    // MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_MERCHANT_ID was previously removed; clean up any legacy rows
                    $db->Execute(
                        "DELETE FROM " . TABLE_CONFIGURATION . "
                          WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_MERCHANT_ID'
                          LIMIT 1"
                    );
                    // Fall through to re-introduce the configuration with validation

                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.7', '<'):
                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.8', '<'):
                ...
                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.9', '<'):
                    // Add configuration options to control button display on cart and product pages
                    $db->Execute(
                        "INSERT IGNORE INTO " . TABLE_CONFIGURATION . "
                            (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
                         VALUES
                            ('Enable on Shopping Cart Page?', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SHOPPING_CART', 'True', 'Do you want to display the Google Pay button on the shopping cart page?', 6, 0, 'zen_cfg_select_option([''True'', ''False''], ', NULL, now()),
                            ('Enable on Product Page?', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_PRODUCT_PAGE', 'True', 'Do you want to display the Google Pay button on product pages?', 6, 0, 'zen_cfg_select_option([''True'', ''False''], ', NULL, now())"
                    );
                    // Fall through to add Google Pay environment setting

                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.10', '<'):
                    ...
                    $db->Execute(
                        "INSERT IGNORE INTO " . TABLE_CONFIGURATION . "
                            (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
                         VALUES
                            ('Google Pay Environment', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ENVIRONMENT', 'TEST', 'Set Google Pay environment for merchant verification. Use TEST for initial setup and screenshots, then switch to PRODUCTION after Google Pay approval. This setting is independent of PayPal sandbox/live mode.', 6, 0, 'zen_cfg_select_option([''TEST'', ''PRODUCTION''], ', NULL, now())"
                    );
                    // Fall through to update configuration descriptions

                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.11', '<'):
                    ...
                    $db->Execute(
                        "UPDATE " . TABLE_CONFIGURATION . "
                            SET configuration_description = 'Display the Google Pay button on the shopping cart page? NOTE: To use this when the user is not logged in, you must set the Google Pay Merchant ID above.',
                                last_modified = now()
                          WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SHOPPING_CART'
                          LIMIT 1"
                    );
                    ...
                    $db->Execute(
                        "UPDATE " . TABLE_CONFIGURATION . "
                            SET configuration_description = 'Display the Google Pay button on product pages? NOTE: To use this when the user is not logged in, you must set the Google Pay Merchant ID above.',
                                last_modified = now()
                          WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_PRODUCT_PAGE'
                          LIMIT 1"
                    );
                    // Fall through to add guest wallet button control

                case version_compare(MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION, '1.3.12', '<'):
        ...
        $db->Execute(
            "UPDATE " . TABLE_CONFIGURATION . "
                SET configuration_value = '$current_version',
                    last_modified = now()
              WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION'
              LIMIT 1"
        );
    }
    ...
    protected function getPayPalAdvancedCheckoutApi(): ?PayPalAdvancedCheckoutApi
    {
        ...
        try {
            ...
            $this->log->write('Google Pay: Error creating PayPalAdvancedCheckoutApi: ' . $e->getMessage());
    ...
    public function selection(): array
    {
        ...
        $hiddenFields =
            ...
            zen_draw_hidden_field('paypalac_googlepay_payload', '', 'id="paypalac-googlepay-payload"') .
            zen_draw_hidden_field('paypalac_googlepay_status', '', 'id="paypalac-googlepay-status"');

        $script = $this->getWalletAssets('jquery.paypalac.googlepay.js');
    ...
    public function pre_confirmation_check()
    {
        $this->paypalCommon->processWalletConfirmation(
            'google_pay',
            'paypalac_googlepay_payload',
            [
                'title' => MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_TEXT_TITLE ?? 'Google Pay',
                'payload_missing' => MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ERROR_PAYLOAD_MISSING ?? 'Payload missing',
                'payload_invalid' => MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ERROR_PAYLOAD_INVALID ?? 'Invalid payload',
                'confirm_failed' => MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ERROR_CONFIRM_FAILED ?? 'Confirmation failed',
                'payer_action' => MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ERROR_PAYER_ACTION ?? 'Payer action required',
            ]
        );
    }
    ...
    protected function recordPayPalOrderDetails(int $orders_id): void
    {
        ...
        // Implementation similar to paypalac but adapted for Google Pay
        if ($orders_id <= 0) {
            return;
        }
    ...
    public function check(): bool
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_STATUS'");
    ...
    public function install()
    {
        ...
        $db->Execute(
            "INSERT INTO " . TABLE_CONFIGURATION . "
                (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
             VALUES
                ('Module Version', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION', '$current_version', 'Currently-installed module version.', 6, 0, 'zen_cfg_read_only(', NULL, now()),
                ('Enable PayPal Google Pay?', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_STATUS', 'False', 'Do you want to enable PayPal Google Pay payments?', 6, 0, 'zen_cfg_select_option([''True'', ''False'', ''Retired''], ', NULL, now()),
                ('Sort order of display.', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', 6, 0, NULL, NULL, now()),
                ('Payment Zone', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', 6, 0, 'zen_cfg_pull_down_zone_classes(', 'zen_get_zone_class_title', now()),
                ('Google Pay Merchant ID (deprecated)', 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_MERCHANT_ID', '', 'DEPRECATED: This setting is no longer used. As of 2025, PayPal SDK no longer accepts the google-pay-merchant-id parameter. Leave blank.', 6, 0, NULL, NULL, now())" 
        );
    ...
    {
        return [
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_VERSION',
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_STATUS',
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SORT_ORDER',
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_ZONE',
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_MERCHANT_ID',
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_SHOPPING_CART',
            'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_PRODUCT_PAGE',
    ...
    {
        $rawMerchantId = defined('MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_MERCHANT_ID') ? trim((string)MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_MERCHANT_ID) : '';
    ...
    {
        global $db;
        $db->Execute("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key LIKE 'MODULE_PAYMENT_PAYPALAC_GOOGLEPAY_%'");
    }
