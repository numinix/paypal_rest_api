...
(function () {
    ...
    // same size as Apple Pay on the shopping cart/product page even
    // before/without the CSS file loading - Apple Pay never sets an inline
    // width at all, and the per-template <style> blocks used to hardcode a
    // different fixed width (228px) than this script's old 100% fill, which
    // is what made Apple Pay and Google Pay render at different widths.
    ...
    function ensureWalletSelectionHidden() {
        ...
        // so the user can click it to select Google Pay.  The native
    ...
    function loadPayPalSdk(config) {
        ...
        if (sharedSdkLoader.promise && sharedSdkLoader.key === desiredKey && paypalNs && typeof paypalNs.Googlepay === 'function') {
            return sharedSdkLoader.promise.then(function () { return getPayPalNamespace(); });
        }

        // SDK already fully loaded (e.g. by the header observer or another template)
        // even when sharedSdkLoader hasn't been set. Reuse it immediately.
        if (paypalNs && typeof paypalNs.Googlepay === 'function') {
    ...
    // Reset the API retry flag on every IIFE execution so each fresh
    // execution can schedule a retry if the Googlepay API is unavailable.
