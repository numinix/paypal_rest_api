...
- **Migration & Versioning:** Maintain versioned installer scripts under `management/installers/numinix_paypal_isu/`. Installer increments the stored `NUMINIX_PPCP_VERSION` and applies configuration diffs idempotently. Future upgrades append migration scripts (e.g., `2024_01_00_add_telemetry_flags.php`) that check the persisted version before execution.
