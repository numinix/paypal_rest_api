...
$deprecatedAdminPages = [
    'customerNuminixPaypalIsuSaveCreds' => 'Removed deprecated PayPal credential sync endpoint.',
];

foreach ($deprecatedAdminPages as $pageKey => $notice) {
    $lookup = $db->Execute(
        "SELECT page_key"
...
$adminPages = [
    'toolsNuminixPaypalIsuSignupLink' => [
        'language_key' => 'BOX_NUMINIX_PAYPAL_SIGNUP_LINK',
        'main_page' => 'FILENAME_PAYPAL_REQUEST_SIGNUP_LINK',
        'page_params' => '',
        'menu_key' => 'tools',
        'display_on_menu' => 'Y',
        'menu_display' => 'Y',
...
foreach ($adminPages as $pageKey => $pageConfig) {
    if (!zen_page_key_exists($pageKey)) {
        zen_register_admin_page(
            $pageKey,
            $pageConfig['language_key'],
            $pageConfig['main_page'],
            $pageConfig['page_params'],
            $pageConfig['menu_key'],
            $pageConfig['display_on_menu'],
            (int) $pageConfig['sort_order'],
            1
        );
    }

    $updates = [
        "language_key = '" . zen_db_input($pageConfig['language_key']) . "'",
        "main_page = '" . zen_db_input($pageConfig['main_page']) . "'",
        "page_params = '" . zen_db_input($pageConfig['page_params']) . "'",
        "menu_key = '" . zen_db_input($pageConfig['menu_key']) . "'",
        "display_on_menu = '" . zen_db_input($pageConfig['display_on_menu']) . "'",
        "sort_order = " . (int) $pageConfig['sort_order'],
    ];

    if ($menuDisplayColumnExists && isset($pageConfig['menu_display'])) {
        $updates[] = "menu_display = '" . zen_db_input($pageConfig['menu_display']) . "'";
    }
...
if (!$menuDisplayColumnExists && isset($messageStack) && is_object($messageStack)) {
    $messageStack->add(
...
$configurationValues = [
    ...
    [
        'key' => 'NUMINIX_PPCP_PARTNER_REFERRAL_LINK',
        'title' => 'Stored Partner Referral Link',
        'value' => '',
        'description' => 'Stores the most recently generated PayPal onboarding referral URL.',
        'sort_order' => 100,
    ],
];
