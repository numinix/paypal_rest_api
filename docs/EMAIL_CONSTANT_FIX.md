...
in paypalSavedCardRecurring.php:2549
```

This happened when the `notify_error()` method tried to send error notifications.

## Root Cause

Line 2549 in `notify_error()` method used the constant without checking if it was defined:
...
Added `defined()` checks with a proper fallback chain:
...
- `tests/UndefinedEmailConstantTest.php` - New test file
...
4. **Email notification constants** (this fix)
