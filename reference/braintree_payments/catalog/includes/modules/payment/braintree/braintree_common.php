...
class BraintreeCommon {
     ...
     * @param array $moduleData Additional data to pass to the transaction
     * @param bool $submitForSettlement Whether to submit the transaction for settlement (true) or just authorize (false)
     * @return bool False on completion (triggers redirect in calling code)
     */
    public function before_process_common($merchantAccountID = '', $moduleData = [], $submitForSettlement = true) {
        global $messageStack;
        ...
        if (empty($_SESSION['payment_method_nonce'])) {
            $reason = 'Payment processing failed: No valid payment nonce.';
        ...
        // Merge merchant account ID with any additional module data
        if (!empty($merchantAccountID)) {
            $moduleData = array_merge($moduleData, ['merchantAccountId' => $merchantAccountID]);
        }
        ...
        $transaction = $this->process_braintree_payment($nonce, $moduleData, $submitForSettlement);
        if ($transaction) {
            ...
            $reason = $_SESSION['braintree_last_error'] ?? 'Payment processing failed. Please try again.';
            $messageStack->add_session('checkout_payment', $reason, 'error');
            $messageStack->add_session('header', 'Your payment was not processed. Please try again.', 'error');
            if ($this->is_ajax()) {
                echo json_encode([
    ...
    public function process_braintree_payment($token, $moduleData = [], $submitForSettlement = true) {
        ...
        try {
            ...
            // Format amount to exactly 2 decimal places for Braintree
            ...
            if ($result->success) {
                ...
                $this->log_debug('process_braintree_payment SUCCESS', [
                    'transaction_id' => $result->transaction->id,
                    'status' => $result->transaction->status,
                    'amount' => $result->transaction->amount ?? 0,
                    'currency' => $transactionCurrency,
                    'card_type' => $result->transaction->creditCard['cardType'] ?? 'Unknown',
                    'payment_instrument_type' => $result->transaction->paymentInstrumentType ?? 'Unknown'
                ]);
                ...
                $_SESSION['braintree_last_error'] = $reason;
                ...
                $errorMessage = 'Unexpected error during payment: ' . $e->getMessage();
                $_SESSION['braintree_last_error'] = $errorMessage;
