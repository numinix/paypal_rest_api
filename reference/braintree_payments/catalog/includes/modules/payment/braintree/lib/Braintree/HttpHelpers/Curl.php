...
class Curl
{
    ...
    {
        $timeoutSeconds = max(1, (int) $config->getTimeout());
        $timeoutMilliseconds = max(1000, $timeoutSeconds * 1000);

        $httpRequest->setOption(CURLOPT_TIMEOUT, $timeoutSeconds);
        $httpRequest->setOption(CURLOPT_CONNECTTIMEOUT, $timeoutSeconds);

        if (defined('CURLOPT_TIMEOUT_MS')) {
            $httpRequest->setOption(CURLOPT_TIMEOUT_MS, $timeoutMilliseconds);
        }

        if (defined('CURLOPT_CONNECTTIMEOUT_MS')) {
            $httpRequest->setOption(CURLOPT_CONNECTTIMEOUT_MS, $timeoutMilliseconds);
        }
