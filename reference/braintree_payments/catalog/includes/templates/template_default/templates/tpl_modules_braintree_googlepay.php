    ...
    function attemptGooglePayInitialization() {
        loadScriptsSequential(scriptQueue)
            ...
            .catch(function (error) {
                console.error('Braintree Google Pay assets failed to load:', error);
                ...
                if (googlePayRetryAttempts < MAX_RETRY_ATTEMPTS) {
                    ...
                    console.log('Google Pay: Retrying in ' + delay + 'ms (attempt ' + googlePayRetryAttempts + ' of ' + MAX_RETRY_ATTEMPTS + ')');
                    setTimeout(attemptGooglePayInitialization, delay);
                } else {
                    // Fail silently after all retries on cart/product pages
                    console.warn('Google Pay: Initialization failed after ' + MAX_RETRY_ATTEMPTS + ' attempts. Button will not be displayed.');
                }
            });
    }
