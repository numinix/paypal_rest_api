...
if (!function_exists('zen_icon')) {
  /**
   * Provides a lightweight Font Awesome icon helper for older Zen Cart versions
   * where zen_icon() is not yet available.
   *
   * @param string $iconName   The Font Awesome icon name, e.g. "caret-right".
   * @param string $title      Optional title/accessible text for the icon.
   * @param string $size       Optional size suffix ("lg", "2x", etc.).
   * @param bool   $ariaHidden Whether to mark the icon as aria-hidden.
   * @param bool   $srOnly     Whether to output the title text in a screen-reader span.
   *
   * @return string Rendered HTML markup for the icon.
   */
  function zen_icon($iconName, $title = '', $size = '', $ariaHidden = true, $srOnly = true)
  {
    $classes = ['fa'];

    if (!empty($iconName)) {
      $classes[] = 'fa-' . preg_replace('/[^a-z0-9\-]/i', '', $iconName);
    }

    if (!empty($size)) {
      $classes[] = 'fa-' . preg_replace('/[^a-z0-9x\-]/i', '', $size);
    }
    ...
    if ($title !== '') {
      $attributes[] = 'title="' . htmlspecialchars($title, ENT_COMPAT, CHARSET, true) . '"';
    }

    $iconHtml = '<i ' . implode(' ', $attributes) . '></i>';

    if ($title !== '' && $srOnly) {
      $iconHtml .= '<span class="sr-only">' . htmlspecialchars($title, ENT_COMPAT, CHARSET, true) . '</span>';
    }
            ...
            case 'edit':
              $heading[] = array('text' => '<h4>' . $cInfo->configuration_title . '</h4>');
                ...
                if ($cInfo->set_function) {
                  $set_function = ltrim(trim($cInfo->set_function), '\"\'');
                  if (preg_match('/^[A-Za-z_][A-Za-z0-9_]*\s*\(/', $set_function)) {
                    eval('$value_field = ' . $set_function . '"' . htmlspecialchars($cInfo->configuration_value, ENT_COMPAT, CHARSET, TRUE) . '");');
                  } else {
