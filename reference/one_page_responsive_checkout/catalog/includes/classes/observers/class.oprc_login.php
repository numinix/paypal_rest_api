...
class ESLLoginObserver extends base 
{
	...
	function update(&$class, $eventID, $paramsArray) {
	  if (OPRC_STATUS == 'true') {
	    ...
	    if (is_object($browser)) {
	      $_SESSION['browser'] = preg_replace("/[^a-zA-Z0-9s]/", "-", strtolower($browser->getBrowser())) . ' ' . floor($browser->getVersion());
	      global $db;
	      $db->Execute("UPDATE " . TABLE_CUSTOMERS . " SET customers_browser = '" . $_SESSION['browser'] . "' WHERE customers_id = " . (int)$_SESSION['customer_id'] . " LIMIT 1;");
	    }    
