...
class ot_gv {
  ...
  function ot_gv() {
    ...
    if (is_null($_SESSION['cot_gv']) || $_SESSION['cot_gv'] == '0' || (isset($_POST['cot_gv']) && $_POST['cot_gv'] == 0)) $_SESSION['cot_gv'] = '0.00';
    $balance = $this->user_has_gv_account($_SESSION['customer_id']);
    if ($_POST['cot_gv'] > $balance) {
      //if ($balance > $order->info['total']) {
        //$_SESSION['cot_gv'] = $_POST['cot_gv'] = $order->info['total'];
      //} else {
      $_SESSION['cot_gv'] = $_POST['cot_gv'] = $balance;
      //}
    }
    $this->checkbox = ($this->user_has_gv_account($_SESSION['customer_id']) > 0 ? MODULE_ORDER_TOTAL_GV_USER_BALANCE . $currencies->format($this->user_has_gv_account($_SESSION['customer_id'])) . '<br />' : '') . '<label for="cot_gv">' . $this->user_prompt . '</label><input type="text" size="6" onkeyup="submitFunction()" name="cot_gv" value="' . number_format((isset($_POST['cot_gv']) && $_POST['cot_gv'] != 0) ? $_POST['cot_gv'] : $_SESSION['cot_gv'], 2) . '" onfocus="if (this.value == \'' . number_format($_SESSION['cot_gv'], 2) . '\') this.value = \'\';" />';
    $this->output = array();
  ...
  function process() {
    ...
    if ($_SESSION['cot_gv']) {
      ...
      if ($od_amount['total'] > 0) {
        ...
        'text' => '-' . $currencies->format($od_amount['total']) . (($_GET['main_page'] == FILENAME_QUICK_CHECKOUT) ? '<br /><a href="#" class="gvRemove"><span class="smallText">remove</span></a>' : ''),
  ...
  function pre_confirmation_check($order_total) {
    ...
    if ($_SESSION['cot_gv'] > 0) {
      ...
      if (preg_match('/[^0-9\.]/', trim($_SESSION['cot_gv']))) {
        $messageStack->add_session($this->code, TEXT_INVALID_REDEEM_AMOUNT, error);
    ...
    if ($_POST['gv_redeem_code']) {
      ...
      if ($gv_result->RecordCount() > 0) {
        ...
        if ( ($redeem_query->RecordCount() > 0) && ($gv_result->fields['coupon_type'] == 'G')  ) {
          $messageStack->add_session($this->code, ERROR_NO_INVALID_REDEEM_GV, error);
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'));
        }
      } else {
      ...
      if ($gv_result->fields['coupon_type'] == 'G') {
        ...
        if ($gv_amount_result->RecordCount() > 0) {
          ...
          $customer_gv = true;
          $_SESSION['cot_gv'] = $total_gv_amount;
        }
        ...
        $messageStack->add_session($this->code,ERROR_REDEEMED_AMOUNT. $currencies->format($gv_amount), 'success' );
