...
class ot_coupon {
  ...
  function process() {
    ...
    if ($this->enabled == true) {
      ...
      if (MODULE_ORDER_TOTAL_COUPON_WHOLESALE == 'true' && isset($_SESSION['customer_whole']) && $_SESSION['customer_whole'] != 0) {
      ...
      //      if ($this->calculate_tax == "Standard") $order->info['total'] -= $tax;
        ...
        $this->output[] = array('title' => $this->title . ' (' . $this->coupon_code . '):',
          'text' => '-' . $currencies->format($od_amount['total']) . (($_GET['main_page'] == FILENAME_QUICK_CHECKOUT || $_GET['main_page'] == FILENAME_ONE_PAGE_CHECKOUT) ? '<br /><a href="#" class="couponRemove"><span class="smallText">remove</span></a>' : ''),
...
//    print_r($od_amount);
  ...
  function credit_selection() {
    global $discount_coupon, $db, $order;
    if ($this->enabled == true) {
      // disable for wholesale
      if (MODULE_ORDER_TOTAL_COUPON_WHOLESALE == 'true' && isset($_SESSION['customer_whole']) && $_SESSION['customer_whole'] != 0) {
        unset($_SESSION['cc_id']);
      ...
      $selection = array('id' => $this->code,
       'module' => $this->title,
       'redeem_instructions' => ((MODULE_ORDER_TOTAL_COUPON_REDEEM_INSTRUCTIONS != '') ? MODULE_ORDER_TOTAL_COUPON_REDEEM_INSTRUCTIONS : ''),
       'fields' => array(array('title' => MODULE_ORDER_TOTAL_COUPON_TEXT_ENTER_CODE . ($discount_coupon->fields['coupon_code'] != '' ? ' (' . MODULE_ORDER_TOTAL_COUPON_TEXT_CURRENT_CODE . $discount_coupon->fields['coupon_code'] . ($_GET['main_page'] == FILENAME_QUICK_CHECKOUT ? '' : '') . ')' : '') . '',
...
return $selection;
  ...
  function collect_posts() {
    ...
    if (isset($_POST['dc_redeem_code']) && strtoupper($_POST['dc_redeem_code']) == 'REMOVE') {
      unset($_POST['dc_redeem_code']);
      unset($_SESSION['cc_id']);
      $messageStack->add_session($this->code, TEXT_REMOVE_REDEEM_COUPON, 'caution');
    }
//    print_r($_SESSION);
    ...
    if ($_SESSION['cc_id'] > 0) {
      $sql = "select coupon_id, coupon_amount, coupon_type, coupon_minimum_order, uses_per_coupon, uses_per_user,
              restrict_to_products, restrict_to_categories, coupon_zone_restriction, coupon_code, manufacturer_ids
              from " . TABLE_COUPONS . "
      ...
      $check_flag = false;
    //echo "select zone_id, zone_country_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . $coupon_result->fields['coupon_zone_restriction'] . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id";
      ...
      if (!$foundvalid) {
        $messageStack->add_session($this->code, TEXT_REMOVE_REDEEM_COUPON_ZONE, 'caution');
        $this->clear_posts();
    ...
    if ((isset($_POST['dc_redeem_code']) && $_POST['dc_redeem_code'] != '') || (isset($discount_coupon->fields['coupon_code']) && $discount_coupon->fields['coupon_code'] != '')) {
      ...
      $sql = "select coupon_id, coupon_amount, coupon_type, coupon_minimum_order, uses_per_coupon, uses_per_user,
              restrict_to_products, restrict_to_categories, coupon_zone_restriction, manufacturer_ids, sales_eligible, specials_eligible
              from " . TABLE_COUPONS . "
      ...
      if ($coupon_result->fields['coupon_type'] != 'G') {
        ...
        if ($coupon_result->RecordCount() < 1 ) {
          $messageStack->add_session($this->code, TEXT_INVALID_REDEEM_COUPON,'caution');
          $this->clear_posts();
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
...
//        if ($order_total['totalFull'] < $coupon_result->fields['coupon_minimum_order']) {
        ...
        if ($coupon_result->fields['coupon_minimum_order'] > 0 && strval($order_total['orderTotal']) < $coupon_result->fields['coupon_minimum_order']) {
          ...
          $messageStack->add_session($this->code, sprintf(TEXT_INVALID_REDEEM_COUPON_MINIMUM, $currencies->format($coupon_result->fields['coupon_minimum_order'])),'caution');
          $this->clear_posts();
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL',true, false));
        }
        ...
        if ($foundvalid == true) {
          ...
          foreach ($products as $product) {
            if (is_product_valid($product['id'], $_SESSION['cc_id'])) {
              // disable for sales?
              if (MODULE_ORDER_TOTAL_COUPON_SALES == 'true') {
                if ($coupon_result->fields['sales_eligible'] != 1 || $coupon_result->fields['specials_eligible'] != 1) {
                  // adjust order total to remove sales and special items
                  $display_special_price = zen_get_products_special_price($product['id'], true);
                  if ($display_special_price && $coupon_result->fields['specials_eligible'] != 1) {
                    $order_total['orderTotal'] -= ($product['final_price'] * $product['quantity']);
                    continue;
                  } elseif (!$display_special_price && $coupon_result->fields['sales_eligible'] != 1) {
                    // no need to check for sales price and specials price, one is enough
                    $display_sale_price = zen_get_products_special_price($product['id'], false);
              ...
              // check for manufacturer restrictions
              if ($coupon_result->fields['manufacturer_ids'] != 0) {
                if (!is_array($manufacturer_ids)) $manufacturer_ids = explode(',', $coupon_result->fields['manufacturer_ids']);
                $manufacturers_id = zen_get_products_manufacturers_id($product['id']);
                if (in_array($manufacturers_id, $manufacturer_ids)) {
                  $order_total['orderTotal'] -= ($product['final_price'] * $product['quantity']);
                  continue;
                }
              }
              // if we got this far, the product must be valid
              $foundvalid = true;
              continue;
            }
          }
        ...
        if (!$foundvalid || $order_total['orderTotal'] == 0) {
          $messageStack->add_session($this->code, TEXT_INVALID_COUPON_PRODUCT, 'caution');
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
        ...
        if ($date_query->RecordCount() < 1 ) {
          $messageStack->add_session($this->code, TEXT_INVALID_STARTDATE_COUPON,'caution');
          $this->clear_posts();
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
        ...
        if ($date_query->RecordCount() < 1 ) {
          $messageStack->add_session($this->code, TEXT_INVALID_FINISHDATE_COUPON,'caution');
          $this->clear_posts();
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
        ...
        if ($coupon_count->RecordCount() >= $coupon_result->fields['uses_per_coupon'] && $coupon_result->fields['uses_per_coupon'] > 0) {
          $messageStack->add_session($this->code, TEXT_INVALID_USES_COUPON . $coupon_result->fields['uses_per_coupon'] . TIMES ,'caution');
          $this->clear_posts();
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
        ...
        if ($coupon_count_customer->RecordCount() >= $coupon_result->fields['uses_per_user'] && $coupon_result->fields['uses_per_user'] > 0) {
          $messageStack->add_session($this->code, sprintf(TEXT_INVALID_USES_USER_COUPON, $dc_check) . $coupon_result->fields['uses_per_user'] . ($coupon_result->fields['uses_per_user'] == 1 ? TIME : TIMES) ,'caution');
          $this->clear_posts();
          if( zen_not_null($_POST['promotion_code']) ) return null; // modified for ajax coupon (numinix ajax shopping cart)
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
...
//              } elseif ($check->fields['zone_id'] == $order->billing['zone_id']) {
          ...
          if (!$foundvalid) {
            $messageStack->add_session($this->code, TEXT_REMOVE_REDEEM_COUPON_ZONE, 'caution');
            $this->clear_posts();
            if (!$foundvalid && !zen_not_null($_POST['promotion_code'])) zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL',true, false)); // modified for ajax coupon (numinix ajax shopping cart)
          } else {
            ...
            if (isset($_POST['dc_redeem_code'])) $messageStack->add('checkout', TEXT_VALID_COUPON,'success');
          }
          // set promotion code for shopping cart
        }
      } else {
        $messageStack->add_session($this->code, TEXT_INVALID_REDEEM_COUPON,'caution');
        $this->clear_posts();
        if( !zen_not_null($_POST['promotion_code']) ) zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL',true, false)); // modified for ajax coupon (numinix ajax shopping cart)
      }
    }
  }
  /**
    ...
    if ($_SESSION['cc_id']) 
    {
      ...
      foreach ($products as $product) {
        if (is_product_valid($product['id'], $_SESSION['cc_id'])) {
          // disable for sales?
          if (MODULE_ORDER_TOTAL_COUPON_SALES == 'true') {
            if ($coupon->fields['sales_eligible'] != 1 || $coupon->fields['specials_eligible'] != 1) {
              // adjust order total to remove sales and special items             
              $display_special_price = zen_get_products_special_price($product['id'], true);
              if ($display_special_price && $coupon->fields['specials_eligible'] != 1) {
                $orderTotalDetails['orderTotal'] -= ($product['final_price'] * $product['quantity']);
                continue;
              } elseif (!$display_special_price && $coupon->fields['sales_eligible'] != 1) {
                // no need to check for sales price and specials price, one is enough
                $display_sale_price = zen_get_products_special_price($product['id'], false);
                if ($display_sale_price) {
                  $orderTotalDetails['orderTotal'] -= ($product['final_price'] * $product['quantity']);
          ...
          // check for manufacturer restrictions
          if ($coupon->fields['manufacturer_ids'] != 0) {
            if (!is_array($manufacturer_ids)) $manufacturer_ids = explode(',', $coupon->fields['manufacturer_ids']);
            $manufacturers_id = zen_get_products_manufacturers_id($product['id']);
            if (in_array($manufacturers_id, $manufacturer_ids)) {
              $orderTotalDetails['orderTotal'] -= ($product['final_price'] * $product['quantity']);
              continue;
...
// left for total order amount vs qualified order amount - $orderTotalFull
   ...
   */
  function keys() {
    return array('MODULE_ORDER_TOTAL_COUPON_MASTER_STATUS', 'MODULE_ORDER_TOTAL_COUPON_STATUS', 'MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', 'MODULE_ORDER_TOTAL_COUPON_INC_SHIPPING', 'MODULE_ORDER_TOTAL_COUPON_INC_TAX', 'MODULE_ORDER_TOTAL_COUPON_CALC_TAX', 'MODULE_ORDER_TOTAL_COUPON_TAX_CLASS','MODULE_ORDER_TOTAL_COUPON_WHOLESALE','MODULE_ORDER_TOTAL_COUPON_SALES');  
  }
  /**
   * Enter description here...
   *
  ...
  function install() {
    global $db;
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('This module is installed', 'MODULE_ORDER_TOTAL_COUPON_MASTER_STATUS', 'true', '', '6', '1','zen_cfg_select_option(array(\'true\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('This module is enabled', 'MODULE_ORDER_TOTAL_COUPON_STATUS', 'true', '', '6', '1','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Disable Wholesale', 'MODULE_ORDER_TOTAL_COUPON_WHOLESALE', 'false', 'Disable coupon codes for wholesale?', '6', '7','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Disable Sales', 'MODULE_ORDER_TOTAL_COUPON_SALES', 'false', 'Disable discounts for sales items?', '6', '7','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Re-calculate Tax', 'MODULE_ORDER_TOTAL_COUPON_CALC_TAX', 'Standard', 'Re-Calculate Tax', '6', '9','zen_cfg_select_option(array(\'None\', \'Standard\', \'Credit Note\'), ', now())");
