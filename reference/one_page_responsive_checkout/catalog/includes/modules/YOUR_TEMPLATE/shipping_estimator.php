...
*/
  ...
  if ( defined('MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING') && (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true')) {
    ...
    if ( ($pass == true) && ($_SESSION['cart']->show_total() >= MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER)) {
      $free_shipping = true;
      include(DIR_WS_LANGUAGES . $_SESSION['language'] . '/modules/order_total/lang.ot_shipping.php');
    }
  } else {
    $free_shipping = false;
  }
  
  // bof modification by Numinix
  // set cheapest first, to record $order for shipping estimator in order_total at checkout when not logged in
  $selected_shipping = $shipping_modules->cheapest();
  // eof modification by Numinix
...
//      $selected_shipping = $shipping_modules->cheapest();
  ...
  if($free_shipping) {
	...
	// bof modification by Numinix
  // record $order for shipping estimator in order_total at checkout when not logged in
  $order->info['shipping_module_code'] = ($order->info['shipping_cost'] != 0) ? $selected_shipping['id'] : '';
  $_SESSION['ac_shipping_estimator_order'] = array();
  $_SESSION['ac_shipping_estimator_order']['info'] = $order->info;
  $_SESSION['ac_shipping_estimator_order']['delivery'] = $order->delivery;
  $_SESSION['shipping'] = ($order->info['shipping_cost'] != 0) ? $selected_shipping : '';
  // eof modification by Numinix
