 ...
 * @version $Id: checkout_new_address.php 11 2012-08-24 18:17:44Z numinix $
 */
...
if ((isset($_POST['action']) && ($_POST['action'] == 'submit')) || (isset($_POST['oprc_change_address']) && ($_POST['oprc_change_address'] == 'submit'))) {
...
// END PO Box Ban 1/1
    ...
    if ($error == false) {
      $sql_data_array = array(array('fieldName'=>'address_title', 'value'=>$address_title, 'type'=>'string'),
      ...
      if (ACCOUNT_TELEPHONE == 'true' || ACCOUNT_TELEPHONE_SHIPPING == 'true') $sql_data_array[] = array('fieldName'=>'entry_telephone', 'value' => $_POST['telephone'], 'type'=>'string');
      if (ACCOUNT_STATE == 'true') {
        if ($zone_id > 0) {
      ...
      if ((int)zen_count_customer_address_book_entries() > (int)MAX_ADDRESS_BOOK_ENTRIES) {
        ...
        $address_check = $db->Execute("SELECT count(address_book_id) as count FROM " . TABLE_ADDRESS_BOOK . " 
                                       WHERE customers_id = " . (int)$_SESSION['customer_id'] . ";");
        
        // update default to current to avoid issues on next visit
   //     $db->Execute("UPDATE " . TABLE_CUSTOMERS . " SET customers_default_address_id = " . (int)$new_id . " WHERE customers_id = " . $_SESSION['customer_id'] . " LIMIT 1;");
  //      $_SESSION['customer_default_address_id'] = $new_id;
        $both = false;
        if ($address_check->fields['count'] < 2) {
          $both = true;
        }
      }
      if (MAX_ADDRESS_BOOK_ENTRIES == 1 || $both) $addressType = 'both';
      switch($addressType) {
        case 'billto':
          ...
          if ($_POST['telephone'] != '') {
            $db->Execute("UPDATE " . TABLE_CUSTOMERS . " SET customers_telephone = '" . $_POST['telephone'] . "' WHERE customers_id = " . $_SESSION['customer_id'] . " LIMIT 1;");
          }
          if (!$_POST['oprc_change_address']) zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', false));
          break;
        case 'shipto':
          ...
          if (!$_POST['oprc_change_address']) zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL', false));
          break;
        case 'both':
          $_SESSION['billto']  = $new_id;
          $_SESSION['sendto'] = $_SESSION['customers_default_shipping_address_id'] = $_SESSION['cart_address_id'] = $new_id;
          ...
          if (!$_POST['oprc_change_address']) zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL', false)); // redirect to shipping since it is first
          break;
      }
    }
