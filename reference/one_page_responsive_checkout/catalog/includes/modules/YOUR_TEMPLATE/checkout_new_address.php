    ...
    if (
        ...
        switch ($addressType) {
            ...
            case 'shipto':
                ...
                $previousDefaultShipping = $_SESSION['customers_default_shipping_address_id'] ?? null;
                $previousCartAddressId = $_SESSION['cart_address_id'] ?? null;

                $reset_shipping = false;
                if (!empty($previousSendTo)) {
                    if ((int)$previousSendTo !== $addressId) {
                ...
                $check_address_query = $db->bindVars($check_address_query, ':customersID', $_SESSION['customer_id'], 'integer');
                $check_address_query = $db->bindVars($check_address_query, ':addressBookID', $addressId, 'integer');
                $check_address = $db->Execute($check_address_query);
                if ($check_address->fields['total'] == '1') {
                    $_SESSION['sendto'] = $addressId;
                    $_SESSION['customers_default_shipping_address_id'] = $addressId;
                    $_SESSION['cart_address_id'] = $addressId;
                    ...
                    $_SESSION['sendto'] = '';
                    if ($previousDefaultShipping !== null) {
                        $_SESSION['customers_default_shipping_address_id'] = $previousDefaultShipping;
                    } else {
                        unset($_SESSION['customers_default_shipping_address_id']);
                    }
                    if ($previousCartAddressId !== null) {
                        $_SESSION['cart_address_id'] = $previousCartAddressId;
                    } else {
