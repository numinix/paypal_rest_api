...
(function () {
    ...
    function loadGooglePayScripts() {
        if (window.googlePayScriptsLoaded) {
            console.log('Google Pay (Cart): Required scripts already loaded');
        ...
        <?php else: ?>
        console.log('Google Pay (Cart): 3DS disabled via module settings; skipping 3DS resources');
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (existing) {
                    if (existing.dataset.loaded === 'true') {
                        console.log('Google Pay (Cart): Script already loaded', src);
                        resolve();
                        return;
                    }
                    ...
                    if (existing.readyState === 'complete' || existing.readyState === 'loaded') {
                        ...
                        console.log('Google Pay (Cart): Script already loaded (from cache)', src);
                        resolve();
                        return;
                    }
                    existing.addEventListener('load', function () {
                        existing.dataset.loaded = 'true';
                        console.log('Google Pay (Cart): Script loaded', src);
                        resolve();
                    });
                    existing.addEventListener('error', function () {
                        console.warn('Google Pay (Cart): Failed to load script', src);
                ...
                script.addEventListener('load', function () {
                    ...
                    console.log('Google Pay (Cart): Loaded script', src);
                    resolve();
                });
        ...
        console.log('Google Pay (Cart): Loading resources', scripts);
        console.log('Google Pay (Cart): Browser detection - iOS Chrome:', isIOSChrome);
        ...
        if (isIOSChrome) {
            console.log('Google Pay (Cart): Using sequential loading for iOS Chrome');
            ...
            console.log('Google Pay (Cart): Using parallel loading for non-iOS Chrome browsers');
    ...
    function initializeGooglePay() {
        window.braintreeGooglePaySessionAppend = window.braintreeGooglePaySessionAppend || "<?php echo (SESSION_RECREATE === 'True') ? ('?' . zen_session_name() . '=' . zen_session_id()) : ''; ?>";
        ...
        braintree.client.create({
            ...
            return braintree.googlePayment.create({
                ...
                function onPaymentAuthorized(paymentData) {
                    ...
                    googlePaymentInstance.parseResponse(paymentData)
                        .then(function (result) {
                            if (!result.nonce) {
                                console.error('Google Pay parse error: no nonce');
                                return;
                            }
                            ...
                            const proceedToCheckout = function (nonce) {
                                ...
                                fetch('ajax/braintree_checkout_handler.php' + window.braintreeGooglePaySessionAppend, {
                                    method: 'POST',
                                    ...
                                    .catch(error => {
                                        ...
                                        if (errorDiv) {
                                            errorDiv.innerText = 'There was an error processing your payment. Please try again.';
                                            errorDiv.style.display = 'block';
                                        }
                                    });
                            };
                    ...
                    switch (intermediatePaymentData.callbackTrigger) {
                        case "INITIALIZE":
                            return getGoogleShippingOptions(shippingAddress)
                                ...
                                .catch(err => {
                                    console.error("Error in onPaymentDataChanged (INITIALIZE):", err);
                                    return {
                                        error: {
                ...
                function clearCartIfNeeded() {
                    return fetchJSON('ajax/braintree_clear_cart.php' + window.braintreeGooglePaySessionAppend, {
                        method: 'POST',
                        redirect: 'manual'
                    })
                        .then(data => {
                ...
                function proceedToCheckoutAfterCartClear(nonce) {
                    return clearCartIfNeeded()
                        .then(() => {
                            showGooglePayLoading();
                            return fetch('ajax/braintree_checkout_handler.php' + window.braintreeGooglePaySessionAppend, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    payment_method_nonce: nonce,
        ...
        function attemptGooglePayInitialization() {
            loadGooglePayScripts()
                ...
                .catch(function (error) {
                    console.error('Google Pay (Cart): Failed to load scripts:', error);
                    ...
                    if (googlePayRetryAttempts < MAX_RETRY_ATTEMPTS) {
                        ...
                        console.log('Google Pay (Cart): Retrying in ' + delay + 'ms (attempt ' + googlePayRetryAttempts + ' of ' + MAX_RETRY_ATTEMPTS + ')');
                        setTimeout(attemptGooglePayInitialization, delay);
                    } else {
                        // Fail silently after all retries on cart/product pages
                        console.warn('Google Pay (Cart): Initialization failed after ' + MAX_RETRY_ATTEMPTS + ' attempts. Button will not be displayed.');
                    }
                });
        }
