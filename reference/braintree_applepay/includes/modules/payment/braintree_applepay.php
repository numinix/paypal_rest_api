    ...
    function selection() {
        ...
        (function () {
            ...
            function checkApplePayAvailability() {
                ...
                // Note: We skip canMakePaymentsWithActiveCard here because it requires a merchant identifier
                // which is only available after the Apple Pay instance is created.
                // The more specific check using applePayInstance.canMakePayments() is performed
                // in handleApplePayInstance() after instance creation.
            ...
            function handleApplePayInstance(container, applePayInstance) {
                ...
                if (typeof applePayInstance.canMakePayments === "function") {
                    return toPromise(applePayInstance.canMakePayments()).then(function (canMakePayments) {
                        applePayDebugLog("applePayInstance.canMakePayments resolved", canMakePayments);
                        if (!canMakePayments) {
                            applePayDebugWarn("applePayInstance.canMakePayments returned false; Apple Pay button will not render");
                            hideApplePayOption();
                            return;
                        }
                        render();
                    }).catch(function (err) {
                        applePayDebugWarn("applePayInstance.canMakePayments errored", err);
                        hideApplePayOption();
                        return;
                    });
                }
